...
# ============================================================================
# SECTION 11: SPEED OF LIGHT c^2 = a1 FROM SPECTRAL GAPS
# ============================================================================

print_section("PART 11: Speed of light c^2 = a1")

# Decompose L = L_fiber + L_cross
A_cross = A - A_fiber
L_fiber_full = np.diag(np.sum(A_fiber, axis=1)) - A_fiber
L_cross_full = np.diag(np.sum(A_cross, axis=1)) - A_cross
L_full = degree * np.eye(N) - A

# Verify decomposition
decomp_err = np.max(np.abs(L_full - L_fiber_full - L_cross_full))
check(decomp_err < TOL, "L = L_fiber + L_cross (exact decomposition)",
      f"Max error: {decomp_err:.2e}")

# Verify degrees
deg_fiber = np.sum(A_fiber, axis=1)
deg_cross = np.sum(A_cross, axis=1)
check(np.all(np.abs(deg_fiber - 2) < TOL), "Fiber degree = 2 (C10 cycles)")
check(np.all(np.abs(deg_cross - 10) < TOL), "Cross-fiber degree = 10 = 2*a1")

# Check commutativity [L_fiber, L_cross] = 0
commutator = L_fiber_full @ L_cross_full - L_cross_full @ L_fiber_full
comm_norm = np.linalg.norm(commutator, 'fro')
check(comm_norm < TOL, "[L_fiber, L_cross] = 0 (commute exactly)",
      f"Frobenius norm: {comm_norm:.2e}")

# Compute spectral gaps (BLIND: compute first, compare after)
evals_Lf = np.sort(eigh(L_fiber_full, eigvals_only=True))
evals_Lc = np.sort(eigh(L_cross_full, eigvals_only=True))
evals_L = np.sort(eigh(L_full, eigvals_only=True))

gap_fiber = evals_Lf[evals_Lf > TOL][0]
gap_cross = evals_Lc[evals_Lc > TOL][0]
gap_full = evals_L[evals_L > TOL][0]

print(f"  gap(L_fiber) = {gap_fiber:.10f}")
print(f"  gap(L_cross) = {gap_cross:.10f}")
print(f"  gap(L_full)  = {gap_full:.10f}")

# Compare with exact values
gap_fiber_exact = 2 - phi     # C10 cycle gap
gap_cross_exact = 10 - 5*phi  # = a1*(2 - phi)
gap_full_exact = 12 - 6*phi   # known 600-cell gap

check(abs(gap_fiber - gap_fiber_exact) < TOL,
      f"gap(L_fiber) = 2 - phi = {gap_fiber_exact:.10f}")
check(abs(gap_cross - gap_cross_exact) < TOL,
      f"gap(L_cross) = 10 - 5*phi = {gap_cross_exact:.10f}")
check(abs(gap_full - gap_full_exact) < TOL,
      f"gap(L_full) = 12 - 6*phi = {gap_full_exact:.10f}")

# The key ratio: c^2 = gap(L_cross) / gap(L_fiber) = a1 = 5
ratio_c2 = gap_cross / gap_fiber
check(abs(ratio_c2 - a1) < TOL,
      f"c^2 = gap(L_cross)/gap(L_fiber) = a1 = {a1}",
      f"Computed ratio: {ratio_c2:.10f}")

# Also verify b1 ratio
ratio_b1 = gap_full / gap_fiber
check(abs(ratio_b1 - b1) < TOL,
      f"gap(L_full)/gap(L_fiber) = b1 = {b1}",
      f"Computed ratio: {ratio_b1:.10f}")

# Verify Box = L_cross - a1*L_fiber
Box_from_lap = L_cross_full - a1 * L_fiber_full
box_diff = np.max(np.abs(Box - Box_from_lap))
check(box_diff < TOL, "Box = L_cross - a1*L_fiber (algebraic identity)",
      f"Max error: {box_diff:.2e}")

print(f"\n  ==> Speed of light: c = sqrt(a1) = sqrt(5) = {np.sqrt(5):.10f}")
print(f"      in lattice units (2*phi - 1 = {2*phi - 1:.10f})")

# Verify c^2 = a1 across ALL fibrations
print(f"\n  Checking c^2 = a1 across all {n_found} fibrations...")
all_c2_match = True
for fib_idx, fib_list in enumerate(all_fibrations):
    Af_test = np.zeros((N, N), dtype=float)
    for fiber in fib_list:
        for i in fiber:
            for j in fiber:
                if i != j and A[i, j] > 0.5:
                    Af_test[i, j] = 1.0
    Ac_test = A - Af_test
    Lf_test = np.diag(np.sum(Af_test, axis=1)) - Af_test
    Lc_test = np.diag(np.sum(Ac_test, axis=1)) - Ac_test

    ef = np.sort(eigh(Lf_test, eigvals_only=True))
    ec = np.sort(eigh(Lc_test, eigvals_only=True))
    gf = ef[ef > TOL][0]
    gc = ec[ec > TOL][0]
    r = gc / gf
    if abs(r - a1) > TOL:
        all_c2_match = False
        print(f"    Fibration {fib_idx}: ratio = {r:.10f} != a1")

check(all_c2_match,
      f"c^2 = a1 = {a1} for ALL {n_found} fibrations",
      "Speed of light is fibration-independent")

# Trace moment identities of Box (fibration-independent)
print(f"\n  Checking trace moment identities of Box...")
import math
evals_box_all = np.sort(eigh(Box, eigvals_only=True))
tr2 = np.sum(evals_box_all**2)
tr3 = np.sum(evals_box_all**3)
tr5 = np.sum(evals_box_all**5)

check(abs(tr2 - N * degree * a1) < 1.0,
      f"Tr(Box^2) = N*deg*a1 = {N*degree*a1}",
      f"Computed: {tr2:.1f}")
check(abs(tr3 - N**2) < 1.0,
      f"Tr(Box^3) = N^2 = {N**2}",
      f"Computed: {tr3:.1f}")
check(abs(tr5 - math.factorial(2*a1)) < 10.0,
      f"Tr(Box^5) = (2*a1)! = {math.factorial(2*a1)}",
      f"Computed: {tr5:.1f}")

# Exact arithmetic verification: all Tr(Box^n) are integers (sqrt5 cancels)
# Use the known algebraic eigenvalues to verify sqrt5 cancellation
from fractions import Fraction as Fr
phi_exact = (Fr(1,2), Fr(1,2))  # (rational, sqrt5_coeff)
exact_evals = [
    ((Fr(-10), Fr(0)), 12), ((Fr(-3), Fr(-3)), 10), ((Fr(-5), Fr(-1)), 6),
    ((Fr(0), Fr(-3)), 16), ((Fr(-5), Fr(1)), 6), ((Fr(5), Fr(-3)), 12),
    ((Fr(0), Fr(0)), 9), ((Fr(-3), Fr(3)), 10), ((Fr(10), Fr(-2)), 3),
    ((Fr(0), Fr(3)), 16), ((Fr(5), Fr(3)), 12), ((Fr(12), Fr(0)), 5),
    ((Fr(10), Fr(2)), 3),
]
...
def pow_zsqrt5(x, n):
    r = (Fr(1), Fr(0))
    for _ in range(n):
        r = (r[0]*x[0]+5*r[1]*x[1], r[0]*x[1]+r[1]*x[0])
    return r

tr3_exact = (Fr(0), Fr(0))
tr5_exact = (Fr(0), Fr(0))
for (a, b), m in exact_evals:
    p3 = pow_zsqrt5((a,b), 3)
    p5 = pow_zsqrt5((a,b), 5)
    tr3_exact = (tr3_exact[0]+Fr(m)*p3[0], tr3_exact[1]+Fr(m)*p3[1])
    tr5_exact = (tr5_exact[0]+Fr(m)*p5[0], tr5_exact[1]+Fr(m)*p5[1])

check(tr3_exact == (Fr(N**2), Fr(0)),
      f"Tr(Box^3) = N^2 EXACT (sqrt5 cancels)",
      f"rational={tr3_exact[0]}, sqrt5={tr3_exact[1]}")
check(tr5_exact == (Fr(math.factorial(2*a1)), Fr(0)),
      f"Tr(Box^5) = (2a1)! EXACT (sqrt5 cancels)",
      f"rational={tr5_exact[0]}, sqrt5={tr5_exact[1]}")

# Dispersion relation: c^2 = a1 at every angular momentum level
print(f"\n  Checking c^2 = a1 per angular momentum level...")
# Simultaneous diagonalization via irrational combination
import numpy as np
M_combo = L_fiber_full + np.pi * L_cross_full
ev_combo, vec_combo = eigh(M_combo)
lf_all = np.array([vec_combo[:, i] @ L_fiber_full @ vec_combo[:, i] for i in range(N)])
lc_all = np.array([vec_combo[:, i] @ L_cross_full @ vec_combo[:, i] for i in range(N)])
mu_all = lc_all - a1 * lf_all

# Check per-level: for each L eigenvalue, average c^2 = a1
L_eigenvalues = [0, 12-6*phi, 10-2*np.sqrt(5), 9, 12, 14,
                 10+2*np.sqrt(5), 15, 6+6*phi]
all_levels_ok = True
for L_val in L_eigenvalues:
    l_full = lf_all + lc_all
    indices = np.where(np.abs(l_full - L_val) < 0.1)[0]
    if len(indices) > 0 and np.mean(lf_all[indices]) > 0.001:
        c2_level = np.mean(lc_all[indices]) / np.mean(lf_all[indices])
        if abs(c2_level - a1) > 0.001:
            all_levels_ok = False

check(all_levels_ok, "c^2 = a1 at every angular momentum level",
      "Dispersion relation exact at all l")

# Mass ratio mu2/mu1 = phi
mu_abs = np.sort(np.abs(mu_all))
mu_nonzero = mu_abs[mu_abs > 0.01]
mu1 = mu_nonzero[0]   # smallest nonzero |mu|
...
if mu2 is not None:
    ratio_m2m1 = mu2 / mu1
    check(abs(ratio_m2m1 - phi) < 1e-6,
          f"Mass ratio |mu2|/|mu1| = phi = {phi:.6f}",
          f"Computed: {ratio_m2m1:.10f}")
    check(abs(mu1 - (6/phi - 2)) < 1e-6,
          f"|mu1| = 6/phi - 2 = {6/phi - 2:.6f}",
          f"Computed: {mu1:.10f}")
    check(abs(mu2 - (a1 - np.sqrt(a1))) < 1e-6,
          f"|mu2| = a1 - sqrt(a1) = {a1 - np.sqrt(a1):.6f}",
          f"Computed: {mu2:.10f}")


# ============================================================================
# SECTION 12: PRODUCT IDENTITY
# ============================================================================

print_section("PART 12: Product identity check")
...
# =====================================================================
# PART 12: McKay mass-distance correspondence (r = 0.93)
# =====================================================================
print()
print("=" * 60)
print("PART 12: McKay mass-distance correspondence")
print("=" * 60)

# Build the extended E8 Dynkin diagram adjacency
...
for i, j in mckay_edges:
    mckay_adj[i, j] = mckay_adj[j, i] = 1

# Kernel irreps: rho_0, rho_1, rho_8
kernel_irreps = {0, 1, 8}

# BFS distance from kernel nodes
from collections import deque
dist_from_kernel = [999] * 9
queue = deque()
for k in kernel_irreps:
    dist_from_kernel[k] = 0
    queue.append(k)
while queue:
    node = queue.popleft()
    for nb in range(9):
        if mckay_adj[node, nb] and dist_from_kernel[nb] > dist_from_kernel[node] + 1:
            dist_from_kernel[nb] = dist_from_kernel[node] + 1
            queue.append(nb)

# Compute mean |mu| per irrep using Laplacian eigenvalues as fingerprint
# The 9 irreps of 2I have known Laplacian eigenvalues:
PHI_val = (1 + np.sqrt(5)) / 2
L_values = {
    0: 12.0,           # rho_0: trivial
    1: 12 - 6*PHI_val, # rho_1: 6*phi' = 6-6*phi (negative)... actually 12-6*phi
    2: 10 - 2*np.sqrt(5),
    3: 9.0,
    4: 12.0,  # wait, rho_0 and rho_4 both have L=12? No.
    5: 14.0,
    6: 10 + 2*np.sqrt(5),
    7: 15.0,
    8: 6 + 6*PHI_val,
}
# The adjacency eigenvalues of the 600-cell are:
# rho_0: 12, rho_1: 6*phi, rho_2: 4*phi, rho_3: 3, rho_4: 0,
# rho_5: -2, rho_6: 4-4*phi, rho_7: -3, rho_8: 6-6*phi
# L = deg - A, so L_val = 12 - adj_eigenvalue
adj_evals = {
    0: 12.0, 1: 6*PHI_val, 2: 4*PHI_val, 3: 3.0, 4: 0.0,
    5: -2.0, 6: 4-4*PHI_val, 7: -3.0, 8: 6-6*PHI_val
}
L_values_correct = {i: 12 - adj_evals[i] for i in range(9)}
irrep_dims_list = [1, 2, 3, 4, 5, 6, 3, 4, 2]

# Simultaneous diagonalization to get (L, mu) pairs per mode
L_full_mat = degree * np.eye(N) - A  # graph Laplacian
M_combo = L_full_mat + np.e * Box  # irrational combo to break degeneracies
evals_M, evecs_M = eigh(M_combo)
L_per_mode = np.array([evecs_M[:,i] @ L_full_mat @ evecs_M[:,i] for i in range(N)])
mu_per_mode = np.array([evecs_M[:,i] @ Box @ evecs_M[:,i] for i in range(N)])

# Group modes by irrep using L eigenvalue fingerprint
irrep_mean_mass = {}
for rho_idx in range(9):
    L_target = L_values_correct[rho_idx]
    d = irrep_dims_list[rho_idx]
    diffs = [abs(L_per_mode[i] - L_target) for i in range(N)]
    indices = sorted(range(N), key=lambda i: diffs[i])[:d**2]
    irrep_mean_mass[rho_idx] = np.mean([abs(mu_per_mode[i]) for i in indices])
irrep_mean_mass = [irrep_mean_mass[i] for i in range(9)]

# Pearson correlation
dists_arr = np.array([dist_from_kernel[i] for i in range(9)], dtype=float)
masses_arr = np.array(irrep_mean_mass)
corr = np.corrcoef(dists_arr, masses_arr)[0, 1]

# Monotonicity check
distances_unique = sorted(set(dist_from_kernel))
mean_by_dist = {}
for d in distances_unique:
    ms = [irrep_mean_mass[i] for i in range(9) if dist_from_kernel[i] == d]
    mean_by_dist[d] = np.mean(ms)

monotone = all(mean_by_dist[distances_unique[i]] <= mean_by_dist[distances_unique[i+1]]
               for i in range(len(distances_unique)-1))

print(f"  Distance from kernel -> mean |mu|:")
for d in distances_unique:
    irreps_at_d = [f"rho_{i}" for i in range(9) if dist_from_kernel[i] == d]
    print(f"    d={d}: {', '.join(irreps_at_d):>25s}  mean |mu| = {mean_by_dist[d]:.4f}")

print(f"  Monotonically increasing: {monotone}")
print(f"  Pearson correlation r = {corr:.6f}")

# Tests
test_name = "McKay mass-distance: monotonically increasing"
if monotone:
...
test_name = "McKay mass-distance: r > 0.9"
if corr > 0.9:
    print(f"  [PASS] {test_name} (r = {corr:.4f})")
    N_PASS += 1
else:
    print(f"  [FAIL] {test_name} (r = {corr:.4f})")
    N_FAIL += 1
...
if N_FAIL == 0:
    ...
    print(f"    c^2 = a1 = 5 (speed of light from spectral gaps)")
    print(f"    [L_fiber, L_cross] = 0 (commute exactly)")
    print(f"    c^2 = a1 exact at every angular momentum level")
    print(f"    Mass ratio |mu2|/|mu1| = phi (golden ratio)")
    print(f"    Tr(Box^3) = N^2, Tr(Box^5) = (2*a1)!")
    print(f"    McKay mass-distance r = {corr:.2f} (monotone)")
