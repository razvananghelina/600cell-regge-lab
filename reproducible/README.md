...
| `verify_gravity_600cell_canonical_data_space_identification.py` | Target-disclosed modular equality test between the 120-dimensional compatible edge sector and the derived unsigned vertex-edge scale image, with arbitrary-strut and one-row corruption controls across all graph/convention variants | potentially minutes |
| `verify_gravity_600cell_rational_data_lift.py` | Mechanically independent exact-rational construction and all-row residual verification of the global 240-direction vertex-scale/strut cell-flex lift, with a distinct one-row incidence corruption attack | potentially minutes |
| `verify_gravity_600cell_local_data_lift.py` | Independent 48-unknown derivation of the universal exact 6-by-8 cell response, exact vertex-star support, corrupted-incidence rejection, and Poincare-gauge reconciliation with the refuted old local block | potentially minutes |
| `verify_gravity_600cell_corrected_strut_carrier.py` | Target-blind exact trapezoid derivation and 100-digit census of the unique geometry-selected 120-column strut response in the dynamic slab, with rational local-block, equivariance and corruption controls | potentially minutes |
| `verify_gravity_600cell_full_scale_strut_carrier.py` | Target-disclosed full scale--strut carrier audit: three new complete exact 600-cell face solves, one-cell underdetermination control, exact rank-240 incidence proof, schedule equivariance and target-blind conditioning census | potentially tens of minutes |
| `verify_gravity_600cell_full_scale_strut_precision.py` | Calibrated precision audit of the full carrier using independent LAPACK SVD drivers and an 80-decimal exact-binary Gram spectrum; distinguishes normal-equation loss from a real weakest-singular defect | potentially tens of minutes |
| `verify_gravity_600cell_full_scale_strut_symbolic_adversarial.py` | Mechanically independent generic two-cell proof using four per-vertex cross-plus-strut solves, exact lateral face connection elimination, Gröbner-ideal equality and time-reversal/relabel/corruption attacks | potentially minutes |
| `verify_gravity_600cell_full_scale_strut_symbolic_gap_resolution.py` | Coordinate-free wedge test of the symbolic connection condition, with denominator/rank-factor separation and exact attacks on the two suspect real linear strata | potentially minutes |
| `verify_gravity_600cell_full_scale_strut_canonical_intersection.py` | Target-blind sector census of the complete scale--strut carrier's intersection with the frozen Regge-action strong-equation graph, using reduced and joined formulas, block equilibration and hostile controls | potentially tens of minutes |
| `verify_gravity_600cell_full_scale_strut_canonical_precision.py` | Target-disclosed 100/160-digit resolution of the complete-carrier intersection using direct sparse multiprecision projection, Flint Gram determinants/minors and a no-refit homogeneous candidate attack | potentially hours |
| `verify_gravity_600cell_corrected_strut_alignment.py` | Confirmatory fixed-ledger comparison of the frozen corrected strut carrier with the pole-Schur lift and both hyperbolic tangent branches in all 14 parity-sector cases | potentially minutes |
| `verify_gravity_600cell_corrected_strut_alignment_adversarial.py` | Post-result independent polar-projector audit of all 42 corrected-strut separations, using direct tangent eigenvectors plus span, conjugation and corruption controls | potentially minutes |
| `verify_gravity_600cell_corrected_strut_canonical_intersection.py` | Calibrated 14-sector nullity census of the difference between the geometry-selected corrected strut graph and the canonical pole-Schur graph, with synthetic, stacked-image, basis and corruption controls | potentially minutes |
| `verify_gravity_600cell_corrected_strut_canonical_intersection_adversarial.py` | Post-result QR/Frobenius lower-bound audit of zero corrected-strut/canonical intersection in all 14 sectors, without decisive SVD or Gram eigenvalues | potentially minutes |
