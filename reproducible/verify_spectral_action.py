...
What this script verifies:
 ...
 11. Gauge group derivation: A5 permutation rep on 12 icosahedron
...
# Reduced coefficients A_k = c_k / (2N) = c_k / 240
A0 = int(round(c0 / (2 * N)))
A1 = int(round(c1 / (2 * N)))
A2 = int(round(c2 / (2 * N)))

print()
print(f"  Reduced coefficients: (A0, A1, A2) = ({A0}, {A1}, {A2})")
record("A0 = 11 = Lucas_5", A0 == 11, f"got {A0}")
record("A1 = 62", A1 == 62, f"got {A1}")
record("A2 = 233 = Fibonacci_13", A2 == 233, f"got {A2}")
record("A0 = 2*a1 + 1", A0 == 2 * a1 + 1, f"2*a1+1 = {2*a1 + 1}")
record("A1 = 2*a1^2 + 2*a1 + 2",
       A1 == 2 * a1 * a1 + 2 * a1 + 2,
       f"poly value = {2 * a1 * a1 + 2 * a1 + 2}")
record("A2 = 6*a1^2 + 15*a1 + 8",
       A2 == 6 * a1 * a1 + 15 * a1 + 8,
       f"poly value = {6 * a1 * a1 + 15 * a1 + 8}")

dioph_lhs = 2 * A1 * A1 + 1
dioph_rhs = 3 * A0 * A2
print(f"  Diophantine identity: 2*A1^2 + 1 = {dioph_lhs}")
print(f"                        3*A0*A2   = {dioph_rhs}")
record("2*A1^2 + 1 = 3*A0*A2",
       dioph_lhs == dioph_rhs,
       f"{dioph_lhs} vs {dioph_rhs}")

# The deficit factors as (a1-5)(8*a1^3 + 20*a1^2 + 16*a1 + 3)
unique_hits = []
for aa in range(2, 51):
    A0_aa = 2 * aa + 1
    A1_aa = 2 * aa * aa + 2 * aa + 2
    A2_aa = 6 * aa * aa + 15 * aa + 8
    if 2 * A1_aa * A1_aa + 1 == 3 * A0_aa * A2_aa:
        unique_hits.append(aa)

record("Diophantine identity singles out a1 = 5 on 2..50",
       unique_hits == [5],
       f"hits = {unique_hits}")

cutoff_num = int(round(c1))
cutoff_den = int(round(2 * c0))
cutoff_ratio = c1 / (2 * c0)
record("c1/(2*c0) = 31/11",
       abs(cutoff_ratio - 31 / 11) < 1e-12,
       f"got {cutoff_num}/{cutoff_den} = {cutoff_ratio:.12f}")
...
# The edge-kernel convention uses 3 for SU(2) and 3'+5 for color.  The 3+5
# construction above is its equally valid Galois-conjugate embedding.
gauge_dim = 1 + 3 + 8
print("  Edge-kernel convention: 12 = 1 + 3 + (3'+5) = 1 + 3 + 8")
