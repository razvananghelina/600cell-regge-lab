#!/usr/bin/env python3
"""Preregistered shifted-tick pseudo-longitudinal persistence test."""
...
OUTPUT = HERE / "gravity_600cell_dust_pseudolongitudinal_shifted.json"
PROTOCOL = ROOT / "docs/gravity/gravity_600cell_dust_pseudolongitudinal_shifted_protocol.md"
...
CURRENT_JSON = HERE / "gravity_600cell_dust_action_york_scale_invariant_adversarial.json"
CURRENT_SOURCE = HERE / "verify_gravity_600cell_dust_action_york_scale_invariant_adversarial.py"
CURRENT_RESULT = ROOT / "docs/gravity/gravity_600cell_dust_action_york_pseudolongitudinal_result.md"
SHIFTED_RESULT = ROOT / "docs/gravity/gravity_600cell_dust_shifted_direct_precision_result.md"
...
PROTOCOL_COMMIT = "7f7e06e"
EXPECTED_HASHES = {
    "protocol": "f09c7450f238774b48425674e5b0800d6ef8ea2fee9f45b82794a0edef0a2375",
    ...
    "current_json": "aee9088c1b0bb1cd4ebec12707014ed187f5c6cbe7ad84cc6cd74db705b8d20c",
    "current_source": "2525ef28bace9c9e2a21ca715c25b0524d11353d3a3f5d649649bebaffe87ba6",
    "current_result": "1196c7155eb1057c8c6159e63ade56846e70395d8321702c4ef839b480bbbd83",
    "shifted_result": "904732e25f2dc49c40557cf3da4daa4da172894f2990433522599435d63fbba0",
...
check("all shifted persistence inputs have exact provenance",
      provenance_ok, str(hashes))
shifted_centered = json.loads(SHIFTED_CENTERED_JSON.read_text())
shifted_direct = json.loads(SHIFTED_DIRECT_JSON.read_text())
shifted_upstream_ok = bool(
    shifted_centered["outcome"] == "SHIFTED_CENTERED_CERTIFIED"
    and shifted_centered["passed"] == shifted_centered["tests"] == 7
    and shifted_direct["outcome"] == "SHIFTED_DIRECT_NEGATIVE_RANK_PERSISTS"
    and shifted_direct["passed"] == shifted_direct["tests"] == 14
    and shifted_direct["complete_cells"] == 16
    and shifted_direct["aggregate_sign_counts"]
    == {"NEGATIVE_RESOLVED": 240, "POSITIVE_RESOLVED": 160}
)
check("the shifted centered and direct-rank controls are preserved",
      shifted_upstream_ok)

spec = importlib.util.spec_from_file_location(
    "global_regge_orbits_york_shifted_pseudolongitudinal", GEOMETRY_SOURCE
)
...
check("the shifted audit independently retains ranks 470/354/4",
...
archive = np.load(SHIFTED_CENTERED_NPZ)
families = {}
...
check("every shifted carrier and both normalization denominators are resolved",
...
# The protocol forbids reading current residual values until the full shifted
# census above has been constructed and classified.
current = json.loads(CURRENT_JSON.read_text())
current_ok = bool(
    current["outcome"] == "SCALE_INVARIANT_DIRECT_REFUTATION_CONFIRMED"
    and current["passed"] == current["tests"] == 9
    and current["label_counts"]["relative_span"]
    == {"NONZERO_RESOLVED": 16}
    and current["label_counts"]["relative_commutator"]
    == {"NONZERO_RESOLVED": 16}
    and set(current["cells"])
    == {f"{parity}_sector{sector}" for parity in PARITIES
        for sector in SELECTED_SECTORS}
)
comparisons = {}
comparison_ok = current_ok
if current_ok:
    for (parity, sector), family in families.items():
        cell_name = f"{parity}_sector{sector}"
        comparisons[cell_name] = {}
        for variant, shifted_record in family["records"].items():
            current_record = current["cells"][cell_name]["variants"][variant]
            current_span = float(current_record["relative_span"])
            current_comm = float(current_record["relative_commutator"])
            shifted_span = shifted_record["relative_span"]
            shifted_comm = shifted_record["relative_commutator"]
            record = {
                "span_ratio_shifted_over_current": shifted_span / current_span,
                "commutator_ratio_shifted_over_current": shifted_comm / current_comm,
                "span_absolute_difference": shifted_span - current_span,
                "commutator_absolute_difference": shifted_comm - current_comm,
            }
            comparison_ok &= all(math.isfinite(value) for value in record.values())
            comparisons[cell_name][variant] = record
check("the current artifact is loaded only post-census and comparisons are finite",
      comparison_ok)

controls_ok = bool(
    provenance_ok and shifted_upstream_ok and gro.tests == gro.passed == 43
    and global_ok and basis_ok and carrier_ok and denominator_ok
    and inequalities_ok and complete and comparison_ok
)
persists = bool(
    controls_ok
...
if not controls_ok:
    outcome = "SHIFTED_PSEUDOLONGITUDINAL_CONTROL_FAILED"
elif persists:
    outcome = "SHIFTED_PSEUDOLONGITUDINAL_DEFECT_PERSISTS"
elif closes:
    outcome = "SHIFTED_PSEUDOLONGITUDINAL_DEFECT_CLOSES"
else:
    outcome = "SHIFTED_PSEUDOLONGITUDINAL_OPEN"
allowed = {
    "SHIFTED_PSEUDOLONGITUDINAL_CONTROL_FAILED",
    "SHIFTED_PSEUDOLONGITUDINAL_DEFECT_PERSISTS",
    "SHIFTED_PSEUDOLONGITUDINAL_DEFECT_CLOSES",
    "SHIFTED_PSEUDOLONGITUDINAL_OPEN",
}
check("the shifted frozen hierarchy assigns exactly one outcome",
...
artifact = {
    ...
    "input_sha256": hashes,
    "upstream_outcomes": {
        "shifted_centered": shifted_centered["outcome"],
        "shifted_direct_rank": shifted_direct["outcome"],
        "current_scale_invariant": current["outcome"],
    },
    ...
    "post_census_comparison": {
        cell: {
            variant: {name: sf(value) for name, value in record.items()}
            for variant, record in variants.items()
        }
        for cell, variants in comparisons.items()
    },
    "classification": {
        "two_tick_temporal_persistence": (
            "DERIVED COMPUTATIONAL / STRUCTURAL"
            if persists else "REFUTED" if closes else "OPEN"
        ),
        "curvature_or_refinement_limit": "NOT TESTED",
        "monotonicity_or_lifetime": "NOT TESTED",
...
if comparisons:
    span_ratios = [
        record["span_ratio_shifted_over_current"]
        for variants in comparisons.values() for record in variants.values()
    ]
    comm_ratios = [
        record["commutator_ratio_shifted_over_current"]
        for variants in comparisons.values() for record in variants.values()
    ]
    print("shifted/current span ratio range:", min(span_ratios), max(span_ratios))
    print("shifted/current commutator ratio range:", min(comm_ratios), max(comm_ratios))
