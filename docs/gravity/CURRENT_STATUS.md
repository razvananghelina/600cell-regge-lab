...
The nested equal-boundary `H4` census is load-bearing only if exclusion of
that branch becomes necessary again.  At the user's explicit direction its
deterministic rerun is deferred at the surviving `2/12` checkpoint rather
than spending another approximately 2.5 hours reconstructing a lost negative
artifact.  The earlier complete no-root execution remains user-witnessed, not
a certified computational theorem.  A future rerun is permitted only when
its result is needed for a concrete route decision.
