...
\usepackage[
    ...
    pdftitle={One Integer, Three Generations: ~38 Standard Model Quantities from a1=5 with Zero Free Parameters},
...
\title{\textbf{One Integer, Three Generations:\\
${\sim}\,38$ Standard Model Quantities from $a_1 = 5$}}

\author{
Razvan-Constantin Anghelina\\
\texttt{contact@webdesignmedia.ro}\\[0.3em]
\href{https://webdesignmedia.ro/hjer_particles/}{Interactive visualization} ~~$\cdot$~~ \href{https://github.com/razvananghelina/One-Integer-Three-Generations}{Reproducible code (GitHub)}\\[1em]
...
\begin{abstract}
We show that ${\sim}\,38$ quantities associated with Standard Model structure---the gauge group, three coupling constants, three generations, nine fermion mass ratios, six mixing angles, two CP phases, the Higgs-to-$W$ mass ratio, the electron-to-Planck mass ratio, a discrete PPN analogue, and more---are traced in this framework to a single integer $a_1 = 5$, the unique positive solution of $a_1! = 4\,a_1(a_1+1)$.  All dimensionless relations are internal to the framework, with zero adjustable parameters; in Planck units, the electron mass is fixed by the hierarchy identity $m_e/m_P = \alpha^{4\phi^2}$ (0.24\%), so only the conversion from Planck units to laboratory units remains external.

From $a_1$ we obtain $\phi = (1+\sqrt{5})/2$, the 600-cell ($120$ vertices), and $E_8$ via the icosian construction.  The audited arithmetic and geometric results are separated below from physical identifications.  In particular, the formula $c_{\mathrm{eff}} = \sqrt{2/a_1}(1-b_1\alpha^2)$ is a phenomenological \textsc{pattern}, not a Verlinde derivation: the actual modular data provide the prefactor and the $SU(2)_3$ identity $\|N_{1/2}\|_F^2=6$, but no propagator or categorical-to-mass map fixes their use in a radiative correction.  The resulting mass fit is therefore a consistency fit, not a zero-input TQFT prediction.  The canonical Fibonacci ordered ring and the exceptional conformal embedding $G_2{}_1\times F_4{}_1\subset E_8{}_1$ are exact structural bridges; they do not by themselves derive Standard-Model matter.

The electroweak exponent is not chosen by hand: an RG-bootstrap cost function over integer pairs $(a_1,n)$ has a unique isolated minimum at $(5,25)$ on the scanned discrete domain, and remains there under broad reweightings and after adding a cosmological term.  Interpreting $\Psi(a_1,n) \propto \exp(-S_{\mathrm{eff}})$ yields an exponentially dominant saddle at $(5,25)$, with the partition function numerically saturated by that saddle to machine precision, suggesting an effective vacuum-selection principle.

Galois conjugation sends Fib to the non-unitary Yang--Lee category, with categorical dimension $-1/\phi$.  The formal global-dimension ratio is exactly $\ln(\mathcal D_{\rm Fib}/\mathcal D_{\rm YL})=\ln\phi$, but interpreting this as a physical visible--dark topological entanglement difference is \textsc{open}: no dark ground state, cut, or non-unitary entropy prescription is constructed.  The complementary unitary factor in $G_2{}_1\times F_4{}_1\subset E_8{}_1$ is $F_4{}_1$, not Yang--Lee.  Particle stability, electromagnetic darkness, and the cosmological formula remain phenomenological claims with the provenance classifications stated below.

\emph{Provenance-qualified tests:} measured mixing angles, oscillation
splittings, the leptonic CP best fit, and the Higgs/coupling/mass agreements
are retrodictions: their comparison values were public before these formulae
were fixed.  The genuinely blind neutrino assets are $m_1=0$, strict normal
ordering, $\sum m_\nu=0.058$~eV, $m_\beta\simeq8.8$~meV, and the
phase-specific $m_{\beta\beta}\simeq3.1$--$3.5$~meV.  The complete
chronology is in \texttt{prediction\_provenance\_ledger.md}.
\end{abstract}
...
The Standard Model of particle physics is the most precisely tested physical theory in history. Yet it contains approximately 19 free parameters---coupling constants, mixing angles, and mass ratios---whose values must be determined experimentally. Why does the fine structure constant equal $1/137.036$ rather than some other value? Why are there exactly three generations of fermions? Why does the Higgs boson weigh 125~GeV? These questions remain unanswered.

We propose a radical answer: \textbf{every} dimensionless parameter of the Standard Model is determined by a single self-referential equation,
\begin{equation}
x^2 = x + 1.
\label{eq:selfreference}
\end{equation}
Its positive root is the golden ratio $\phi = (1+\sqrt{5})/2$, the simplest number that equals one plus its own inverse.  Interpreted as a fusion rule---$\tau \otimes \tau = \mathbf{1} \oplus \tau$---it describes a particle that, upon self-interaction, produces the vacuum plus a copy of itself.  Realized as a topological quantum field theory ($SU(2)$ Chern--Simons), this self-referential structure selects a \emph{unique} integer $a_1 = 5$ through a bootstrap self-consistency condition: the quantum dimension of the theory must equal the algebraic parameter it defines.

From $a_1 = 5$ we derive $\phi$, the 600-cell polytope (120 vertices), the $E_8$ root system (240 roots), the gauge group $SU(3) \times SU(2) \times U(1)$, two gauge coupling constants algebraically and a third structurally, the number of fermion generations ($N_{\text{gen}} = 3$), the masses of all nine charged fermions, and the mixing angles of both CKM and PMNS matrices.  The mass spectrum is a \emph{fractal} consequence of~\eqref{eq:selfreference}: every fermion mass is $m_e \cdot \phi^n$, and every power of $\phi$ decomposes as $\phi^n = F_n\,\phi + F_{n-1}$ (Fibonacci), reproducing the same self-referential pattern at every scale.

The logical flow is shown in Figure~\ref{fig:flow}: from the single root $a_1 = 5$, three algebraic branches ($\phi$, $b_1$, $N$) seed the geometric, spectral, and Galois structures that determine all physical parameters.
...
Several programs share the philosophy that Standard Model structure should follow from mathematical constraints rather than free parameters. Lisi \cite{lisi2007} proposed embedding all gauge fields and fermions in $E_8$; Furey \cite{furey2016} derived gauge quantum numbers from the algebra $\mathbb{C} \otimes \mathbb{H} \otimes \mathbb{O}$; and Chamseddine, Connes, and Marcolli \cite{ccm2007} showed that the spectral action on a noncommutative geometry reproduces the full Standard Model Lagrangian coupled to gravity. The input $a_1=5$ determines the 600-cell, $E_8$, and several exact candidate carriers.  It does \emph{not} yet determine the Standard-Model finite algebra, matter module, or finite Dirac operator; Section~\ref{sec:scope_boundaries} gives the certified boundary.

\paragraph{Guiding principle.}
The equation $x^2=x+1$ is exactly the based fusion-ring relation of Fib.  Matching its Perron--Frobenius dimension to the fundamental $SU(2)_{n-2}$ dimension gives the unique integer solution $n=5$, but this is the classical pentagon identity conditional on a chosen matching equation, not a dynamical physical selection principle.  The Fibonacci tower has the same ordered ring $\mathbb Z[\phi]$ canonically; interpreting its levels as masses is \textsc{structural}.  The Galois-conjugate root supplies exact non-unitary modular data, but identifying it with dark particles or electromagnetic couplings is \textsc{pattern/open}.

The paper follows a single logical chain: $a_1 = 5$ (Section~\ref{sec:integer}) $\to$ 600-cell geometry (Section~\ref{sec:geometry}) $\to$ gauge group (Section~\ref{sec:gauge}) $\to$ coupling constants (Section~\ref{sec:coupling}) $\to$ $N_{\text{gen}} = 3$ (Section~\ref{sec:generations}) $\to$ fermion masses (Section~\ref{sec:masses}) $\to$ electroweak bosons (Section~\ref{sec:ew}) $\to$ gravity and spectral action (Section~\ref{sec:gravity}) $\to$ neutrinos (Section~\ref{sec:neutrinos}) $\to$ mixing angles (Section~\ref{sec:mixing}) $\to$ CP violation (Section~\ref{sec:cp}) $\to$ testable predictions (Section~\ref{sec:predictions}).  Extensions to the dark sector, TQFT entanglement, the cosmological constant, and inflation are summarized in Section~\ref{sec:extensions} with full details in the Supplementary Material.

At the end of this chain, the framework admits a second layer of selection: the same data that determine the electroweak and cosmological sectors define an effective cost functional with a sharply isolated minimum at the observed vacuum $(a_1,n) = (5,25)$ on the scanned domain.  We treat this as an effective vacuum-selection principle, not yet as a microscopic Euclidean path integral derivation.


%==============================================================
\section{The Unique Integer}
\label{sec:integer}
%==============================================================

\subsection{Self-Reference and the Golden Ratio}

The framework begins with the observation that the simplest self-referential algebraic equation,
\[
x^2 = x + 1,
\]
already encodes the structure of a topological quantum field theory.  Its positive root $\phi = (1 + \sqrt{5})/2$ satisfies $\phi = 1 + 1/\phi$: the whole equals one plus its own inverse.  Reinterpreted as a fusion rule for anyonic excitations,
\begin{equation}
...
The registered matching equation has a unique integer solution.  Each positive integer $a_1 \ge 3$ determines an algebraic number $\phi(a_1) = (1 + \sqrt{a_1})/2$ and an $SU(2)$ Chern--Simons theory at level $k = a_1 - 2$.  Its fundamental, $j=\tfrac12$, quantum dimension is
\begin{equation}
d_{1/2}(a_1) = 2\cos\!\Big(\frac{\pi}{a_1}\Big).
\end{equation}
...
\begin{equation}
\label{eq:bootstrap}
\boxed{d_{1/2} = \phi(a_1) \quad\Longleftrightarrow\quad 2\cos\!\Big(\frac{\pi}{a_1}\Big) = \frac{1+\sqrt{a_1}}2}
\end{equation}

\begin{theorem}[Conditional pentagon matching]
\label{thm:bootstrap}
The only positive integer $a_1$ satisfying~\eqref{eq:bootstrap} is $a_1 = 5$.
\end{theorem}

\begin{proof}
For $a_1\ge9$, the right side is at least $2$, while $2\cos(\pi/a_1)<2$.  Exact algebraic evaluation for $3\le a_1\le8$ leaves only $a_1=5$, where $2\cos(\pi/5)=(1+\sqrt5)/2$ is the pentagon identity.  The exact finite certificate is in \texttt{verify\_modular\_tqft\_layer.py}.
\end{proof}

\begin{corollary}[Spin-one coincidence at the solution]
\label{cor:double}
At $a_1=5$, the $j=1$ dimension also equals $\phi$, giving dimensions $\{1,\phi,\phi,1\}$.
\end{corollary}

\begin{proof}
Substitution gives \(d_1=\sin(3\pi/5)/\sin(\pi/5)=2\cos(\pi/5)=\phi\).
\end{proof}
...
\end{enumerate}
The equality is exact and unique after the matching prescription is chosen.  Status: uniqueness \textsc{derived}; choice of prescription \textsc{structural}; physical vacuum selection \textsc{open}.  It does not constitute a dynamical selection principle.
...
$m_e/m_P$ & $\alpha^{z_P}$, $z_P = 12\phi^2/3$ (spectral weight of $R_3$) & $\alpha^{4\phi^2}$ & Planck hierarchy \\
\bottomrule
\end{tabular}
\caption{The cascade from $a_1 = 5$. Every entry is determined, with zero free parameters.}
\label{tab:cascade}
\end{table}
...
\subsection{The 600-Cell Polytope}

The 600-cell $\{3,3,5\}$ is a regular 4-dimensional polytope:
...
%==============================================================
\section{Gauge Group from \texorpdfstring{$D_4$}{D4}}
\label{sec:gauge}
%==============================================================
...
Now embed $A_5\subset SO(3)\subset SU(3)$ via the $\mathbf{3'}$-irrep, matching the edge-kernel convention.  The adjoint of $SU(3)$ restricts to $A_5$ as
\begin{equation}
...
\end{tabular}
\end{center}
The vertex kernel sits at the \emph{endpoints} of the $E_8$ Dynkin diagram (the Galois pair $\rho_1, \rho_8$); the edge kernel sits at the \emph{branching node} ($\rho_5$); the face kernel reduces to the trivial representation alone.  All three operators share the pattern $\ker \ni \rho_0$ (a universal vacuum mode).  The gauge content $1 + 3 + 8 = 12$ emerges exclusively at $p = 1$, while $c^2 = a_1$ is a vertex-level identity.  The trace ratios $\operatorname{Tr}(\Box_1^2)/\operatorname{Tr}(\Box_0^2) = 9\,a_1 = 45$ and $\operatorname{Tr}(\Box_2^2)/\operatorname{Tr}(\Box_0^2) = 34$ are exact integers.
...
\end{equation}
All three numbers are therefore internal to the 600-cell with Hopf fibration: $N=|2I|$ is combinatorial, $K_{\mathrm{YM}} = b_1$ is gauge-theoretic, and $\lambda_1(L_{\mathrm{fiber}})$ is spectral.  This proves the displayed finite spectral identity, but not by itself its identification with the coefficient of a physically normalized electromagnetic field: rescaling the map from discrete cochains to $A_{\rm phys}$ leaves these spectral data unchanged and rescales the quadratic action.  A normalized discrete-to-continuum $U(1)$ field map (and a fixed charge unit) is therefore the minimal additional structural axiom.
...
\label{eq:alpha0_box}
\end{equation}
The gauge-sector spectral action on $\Box$ thus produces the displayed tree-level \emph{spectral number} directly, with the ratio $\mathrm{Tr}(\Box^2_{\mathrm{full}})/\mathrm{Tr}(\Box^2_{\mathrm{gauge}}) = a_1$ confirming that the gauge sector is exactly $1/a_1$ of the full spectrum.  Its reading as $1/\alpha_0$ remains structural for the normalization reason just stated.
...
\end{tabular}
\end{center}
Only $k = 0, 1, 3$ satisfy (i)--(ii); condition (iii) eliminates $k=0$; among $k=1$ and $k=3$, the latter has the \emph{minimal} integer prefactor ($c = 2$ vs.\ $c = 8$), selecting it uniquely.  Note that $\text{Tr}(\phi^3) = 4$ equals the spacetime dimension (the 600-cell lives in $\mathbb{R}^4$), and $k = 3 = N_{\text{gen}}$.  The norm follows as a consequence:
\begin{equation}
...
\textbf{Physical interpretation.} The trace condition ties $\alpha_s$ to the gauge sector ($\text{Tr} = \dim(SU(3)) = \text{rank}(E_8)$).  The factor $c = 2 = \text{rank}(E_8)/\text{Tr}(\phi^3)$ is the ratio of the gauge rank to the spacetime dimension.  The negative norm $N = -4$ means $z' < 0$: the Galois conjugate sector has opposite sign, reflecting asymptotic freedom of $SU(3)$.

\textbf{Hierarchy identity.}  The $\alpha_s$ exponent $z = 2\phi^3$ and the Planck exponent $z_{\text{Planck}} = 4\phi^2$ (derived in Section~\ref{sec:spectral_weight}) are related by an exact algebraic identity:
\begin{equation}
\boxed{z_{\text{Planck}} = \frac{1}{\alpha_s} + 2}
\label{eq:hierarchy_identity}
\end{equation}
since $4\phi^2 = 2\phi^3 + 2$.  This links all three forces: $m_e/m_P = \alpha^{1/\alpha_s + 2}$.  In $\mathbb{Z}[\phi]$, $1/\alpha_s = 2 + 4\phi$ and $z_{\text{Planck}} = 4 + 4\phi$ share the same irrational part $4\phi$ (universal geometry) and differ only in the rational part ($2$ vs.\ $4$).
...
The following formulas are algebraic functions of $a_1 = 5$ alone (with $\pi$ from the Hopf fiber phase); the $\alpha$ row is structural because its physical field normalization is not yet derived:
...
\textbf{Remark on the DSI potential.} The scaling factor $\phi$ is derived from the edge dihedral angle $\pi/a_1$, and the $\sin^2$ shape is now derived from the nearest-neighbor structure of $\mathbb{Z}[\phi]$.  In the logarithmic coordinate $x = \ln|\psi|/\ln\phi$, the DSI lattice $\{x \in \mathbb{Z}\}$ has a natural tight-binding Hamiltonian $H = -t\sum_n(|n\rangle\langle n\!+\!1| + \text{h.c.})$.  The Bloch dispersion $E(k) = -2t\cos k$ gives an effective potential $V(x) = 2t(1 - \cos 2\pi x) = 4t\sin^2(\pi x)$---which \emph{is} the $\sin^2$ form.  Crucially, only \emph{nearest-neighbor} hopping contributes because $\phi$ is the fundamental unit of $\mathbb{Z}[\phi]$: next-nearest-neighbor coupling $\phi^2 = \phi + 1$ is reducible (a sum, not a generator).  An independent argument from the heat kernel on the Hopf $S^1$ fiber confirms this: at the natural time scale $t \sim \phi^2$, the $m = 2$ Fourier mode is suppressed by a factor $e^{-3\phi^2} \approx 4 \times 10^{-4}$ relative to $m = 1$, making $\sin^2$ an excellent approximation.  The generation count $N_{\text{gen}} = 3$ does not depend on the shape choice, but the $\sin^2$ form is now the \emph{unique} prediction of the nearest-neighbor lattice dynamics.
...
\bottomrule
\end{tabular}
\caption{Bare fermion mass spectrum from $a_1 = 5$. Leptons and up quark are within $4\%$; heavy quarks deviate $10$--$20\%$. The later correction formula gives the reported fit, but its TQFT/radiative interpretation is \textsc{pattern}.}
\label{tab:masses}
\end{table}
...
\end{equation}
This coefficient links the Weinberg angle to the number of generations: algebraically $C=\tfrac23\sin^2\theta_W=4/(a_1^2+1)$.  Interpreting the numerator as a spacetime dimension $d_{\mathrm{ST}}=4$ is \textbf{STRUCTURAL}, not derived by the static spectrum: the calibrated finite complex is spectrally approximately three-dimensional and the vertex-index route to four dimensions is \textbf{OPEN} (Section~\ref{sec:audited_consolidation}).

\textbf{The correction formula.}  The correction mechanism depends on the algebraic nature of each fermion in $\mathbb{Z}[\phi]$: up quarks ($T_3 = +\tfrac{1}{2}$) couple positively to the algebraic norm $|N(z)|$, down quarks ($T_3 = -\tfrac{1}{2}$) couple negatively and are suppressed by $|\phi'/\phi| = 1/\phi$ (the Galois conjugate ratio), and leptons couple to the Galois conjugate $z'$ directly via an anomalous-dimension exponent $k = 3/4 = 1 - 1/d_{\text{ST}}$.  The corrected mass is $m_f = m_e \cdot \phi^{n_f + \delta_f}$, where:
\begin{equation}
...
\bottomrule
\end{tabular}
\caption{Fermion masses with the phenomenological correction formula (Eq.~\eqref{eq:oneloop}).  The numerical fit is reported, but the modular-data audit classifies the radiative interpretation as \textsc{pattern}.}
...
\end{equation}
The exponent $k = 3/4$ is derived:
\begin{equation}
\boxed{k = 1 - \frac{1}{\dim(\text{spacetime})} = 1 - \frac{1}{a_1 - 1} = \frac{3}{4}}
\label{eq:lepton_k}
\end{equation}
where $\dim(\text{spacetime}) = a_1 - 1 = 4$ (Section~\ref{sec:integer}).  The derivation proceeds through three steps:

\emph{Step 1: Morrey embedding.}  By Morrey's inequality, the Sobolev space $W^{1,d}(\mathbb{R}^1)$ embeds continuously into the H\"{o}lder space $C^{0,\,1-1/d}(\mathbb{R}^1)$, so any function in $W^{1,d}$ satisfies $|f(x) - f(y)| \leq C_M \|f'\|_{L^d} |x-y|^{1-1/d}$.  With $d = d_{\text{ST}} = \mathrm{Tr}(\phi^3) = 4$, the H\"{o}lder exponent is $1 - 1/4 = 3/4$.

\emph{Step 2: Summability fixes $d$.}  The spectral triple $(\mathcal{A}, \mathcal{H}, D)$ is $d_{\text{ST}}$-summable: $|D|^{-1} \in \mathcal{L}^{d_{\text{ST}},\infty}$.  All fluctuations of $D$ are constrained to the $d_{\text{ST}}$-th Schatten class, so the mass correction $\delta(z')$---a function of the single real variable $z' = \sigma(z)$---lies in $W^{1,d_{\text{ST}}}(\mathbb{R}^1)$.

\emph{Step 3: Saturation for leptons ($G_k = 0$).}  Quarks (BLACK nodes on the McKay graph) have $G_k \neq 0$, providing specific spectral correction mechanisms (Arakelov height, Galois cross-term, Cayley eigenvalue) that constrain $\delta$ to be \emph{more} regular than the Morrey bound---the bound is not saturated.  Leptons (WHITE nodes) have $G_k = 0$: the Galois anti-symmetric eigenvalue \emph{vanishes}, and no spectral correction mechanism remains.  The mass correction is therefore determined entirely by the regularity limit of $W^{1,4}(\mathbb{R}^1)$.  The unique extremal function of the one-dimensional Morrey embedding (up to a multiplicative constant) is $f^*(z') = C|z'|^{1-1/d}$, which lies at the boundary of $W^{1,d}$ (its gradient $|z'|^{-1/d}$ has $L^d$-norm that diverges logarithmically).  Saturation follows: $G_k = 0$ removes all constraints beyond regularity, forcing the correction to the extremal function.

Equivalently, $z'$ propagating on the internal Galois space (dimension~$1$) embedded in $d$-dimensional spacetime acquires the anomalous scaling $|z'|^{1-1/d}$---the ratio of spatial to total dimensions.
...
\textbf{Derivation of the coefficient.} The lepton coefficient is uniquely determined by the same spacetime dimension that fixes $k$:
\begin{equation}
\boxed{c_\ell = \frac{C \cdot \phi^3}{\mathrm{Tr}(\phi^3)} = \frac{C \phi^3}{4}}
\label{eq:cell}
\end{equation}
where $\mathrm{Tr}(a + b\phi) = 2a + b$ is the $\mathbb{Z}[\phi]$ trace.  The key identity is $\mathrm{Tr}(\phi^3) = 4 = \dim(\text{spacetime})$: the Lucas sequence $\mathrm{Tr}(\phi^n) = \{1, 1, 3, 4, 7, 11, 18, \ldots\}$ takes the value $d = a_1 - 1 = 4$ \emph{uniquely} at $n = 3$, singling out $\phi^3$ from all powers of $\phi$.  Thus $k = 1 - 1/d$ and $c_\ell = C\phi^3/d$ are not independent choices: both follow from $\mathrm{Tr}(\phi^3) = d$.
...
\textbf{Post-audit assessment.}  The displayed correction formula and fit are retained as a \textsc{pattern}.  The modular calculation proves only that normalized \(SU(2)_3\) has \(S\)-matrix prefactor \(\sqrt{2/5}\) and \(\|N_{1/2}\|_F^2=6\) (Fib itself gives \(3\)).  No propagator, fermion-label map, or categorical contraction derives \(c_{\mathrm{eff}}\).  The fit is therefore not a zero-input TQFT prediction.
...
\textbf{Sobolev--Arakelov interpretation.}  The three functional forms---logarithmic, power-law, constant---are unified by the Morrey--Sobolev embedding theorem.  Define the \emph{effective dimension} $d_{\text{eff}}(z)$ as the Galois rank of the Wilson line $z \in \mathbb{Z}[\phi]$:
\begin{equation}
d_{\text{eff}} =
\begin{cases}
0 & \text{if } b = 0 \text{ (rational, Galois-fixed)}, \\
1 & \text{if } |N(z)| = 1,\; b \neq 0 \text{ (unit, Dirichlet rank 1)}, \\
d_{\text{ST}} = 4 & \text{if } |N(z)| > 1 \text{ (non-unit, full spectral dimension)}.
\end{cases}
\label{eq:d_eff}
\end{equation}
By the Morrey embedding $W^{1,p}(\mathbb{R}^d) \hookrightarrow C^{0,\alpha}$ at $p = d_{\text{ST}} = 4$, the H\"{o}lder exponent is $\alpha = 1 - d_{\text{eff}}/d_{\text{ST}}$, giving:
\begin{equation}
\boxed{\alpha(d_{\text{eff}}) = 1 - \frac{d_{\text{eff}}}{d_{\text{ST}}} \;:\quad
\alpha(0) = 1\;\text{(constant)},\quad
\alpha(1) = \tfrac{3}{4}\;\text{(power)},\quad
\alpha(4) = 0\;\text{(logarithmic)}}
\label{eq:morrey_alpha}
\end{equation}
The three cascade levels are therefore the three regimes of the \emph{same} Sobolev embedding at decreasing regularity.  The spectral action term $\mathrm{Tr}(D^4)$ at order~$\Lambda^0$ provides the $p = 4$ Sobolev energy, and the Galois anti-symmetric inner fluctuation $A_- = (A - \sigma(A))/2$ generates the corrections (vanishing for $b = 0$, as required).  The identity $d_{\text{ST}} = \mathrm{Tr}(\phi^3) = \phi^3 - 1/\phi^3 = 4$ connects the spectral dimension to the golden ratio, making $\phi^3/d_{\text{ST}} = \phi^6/(\phi^6 - 1)$ a derived quantity.  See Supplementary Material~S5.10 for the full derivation chain and gap analysis.
...
\label{eq:oneloop}
\end{equation}
where $\delta_1$ is the tree-level correction from Eq.~\eqref{eq:normlog}.  The proposed one-loop/vertex interpretation of \(c_{\mathrm{eff}}\) is \textsc{pattern}; modular data alone do not define these QED contributions.

\subsubsection*{One-loop coefficient: $c = \sqrt{2/a_1}$}

The normalized $SU(2)_3$ modular matrix has prefactor $\sqrt{2/(k+2)}=\sqrt{2/a_1}$.  This exact normalization motivates, but does not derive, the phenomenological coefficient
\begin{equation}
c_{\mathrm{bare}} = \sqrt{\frac{2}{a_1}} = \sqrt{\frac{2}{5}} = 0.63246
\label{eq:cqed}
\end{equation}
The remaining sine and eigenvalue factors depend on the internal labels; universality does not follow without a specified propagator and contraction.  Status: \textsc{pattern}.
...
\label{eq:vertex}
\end{equation}
The following Frobenius-norm identity is exact category theory; identifying it with radiative phase space is an additional \textsc{pattern}.
...
\end{equation}
This is an algebraic integer concordance, not an independent physical characterisation.  No map relates fusion channels to the separately computed Betti number.

\textbf{Order $\alpha^2$.}  Modular data do not imply the QED Ward--Takahashi identity or perturbative order.  The coefficient extracted near \(6\) at \(O(\alpha^2)\) is a data-conditioned \textsc{pattern} subject to look-elsewhere effects; \(\mathrm{Tr}(N_{1/2})=0\) does not derive a loop order.
...
\end{equation}
as a phenomenological consistency fit.  Table~\ref{tab:masses} gives the comparison; Supplementary Material~S5.7--S5.8 records the original motivation and its post-audit downgrade.
...
\textbf{Physical interpretation.} The trace condition $\text{Tr}(z) = 12 = 2(a_1+1)$ states that gravity ``sees'' the full connectivity of spacetime (both $\phi$- and $\phi'$-sectors contribute). The norm condition $N(z) = 16 = (a_1-1)^2 = \dim(\text{spacetime})^2$ ties the gravitational coupling to the square of the spacetime dimension (equivalently, $\sqrt{N} = 4 = \dim(\mathbb{R}^4)$). This also equals the multiplicity of the central eigenvalue $\lambda_3 = 3$, and the number of fermions per generation.

The gravitational coupling follows immediately:
\begin{equation}
...
\end{equation}
tying the gravitational coupling strength directly to the generation symmetry.

\textbf{Remark.}  In Planck units ($m_P = 1$), the electron mass $m_e = \alpha^{4\phi^2}$ is a pure number derived from the framework with zero free parameters.  The use of $m_e$ as dimensional anchor throughout this paper is a practical convention, not a theoretical necessity---all masses are determined as dimensionless ratios of $m_P$.
...
In the present framework, the hierarchy is a \emph{geometric identity}: $m_e/m_P = \alpha^{z_{\text{Planck}}}$ where both $\alpha$ (from Eq.~\ref{eq:alpha}) and $z_{\text{Planck}} = 4\phi^2$ (from the Proposition above) are derived from~$a_1 = 5$.  There are no free parameters to tune.  The ``large number'' $m_P/m_e \sim 10^{22}$ is simply $(1/\alpha)^{z_{\text{Planck}}} \approx 137^{10.5}$---a moderate base raised to a moderate power, no more ``unnatural'' than $2^{10} = 1024$.  The Galois conjugate exponent $z'_{\text{Planck}} = 6 - 2\sqrt{5} \approx 1.53$ with $\alpha' \approx 21.8$ yields $(\alpha')^{z'} \sim 10^2$: the dark sector has \emph{no hierarchy}.  This asymmetry between visible and dark sectors is itself a consequence of the Galois norm $z \cdot z' = 16 = (a_1 - 1)^2$.

\subsection{The Spectral Weight and the Electromagnetic--Gravitational Link}
\label{sec:spectral_weight}

The Planck exponent has a spectral origin.  For each irrep $\rho_k$ of $2I$ with dimension $d_k$ and character $\chi_k$ at the nearest-neighbor class, define the \emph{spectral weight} $z_k = 12\chi_k^2/d_k$.  The Planck exponent is the spectral weight of $R_3$ (the weak-sector irrep, $\dim = 3$):
\begin{equation}
\boxed{z_{\text{Planck}} = z_{R_3} = \frac{12\,\phi^2}{3} = 4\phi^2}
\label{eq:zP_spectral}
\end{equation}
The linear coefficient of the $\alpha$ equation~\eqref{eq:alpha} factorizes as $B = 4a_1\phi^4 = a_1 \cdot \phi^2 \cdot z_{\text{Planck}}$, so $z_{\text{Planck}}$ appears in two independent roles: as the Planck exponent and as a factor defining $\alpha$.  The mass-direction element satisfies $z_{\text{mass}} = z_{\text{Planck}} + 1/(2\alpha_s)$, with norm $N(z_{\text{mass}}) = 19$ (prime).  The total spectral weight is $\sum z_k = 2a_1^2 = 50$.  See Supplementary Material~S6 for the full spectral weight table and the convergence analysis.
...
Having derived the Planck mass from spectral data, we now derive the gravitational field equation itself from the simplicial structure of the 600-cell.
...
In Regge calculus~\cite{regge1961}, the gravitational variables are edge-length perturbations $h_e$, $e = 1, \ldots, 720$. Under infinitesimal diffeomorphisms $\xi \in \mathbb{R}^V$, the gauge transformation is $h \to h + d_0\,\xi$. Therefore:
\begin{itemize}
\item \textbf{Gauge DOF} = $\dim(\mathrm{Im}(d_0)) = N - 1 = 119$ (exact forms)
\item \textbf{Physical DOF} = $\dim(\mathrm{Im}(d_1^T)) = 601$ (coexact forms)
\end{itemize}

\begin{equation}
\boxed{601 = C + 1 = 600 + 1 = a_1 N + 1}
\end{equation}
The 600 local modes correspond to curvature at each tetrahedral cell (one deficit-angle degree of freedom per cell), plus one global conformal mode (the overall $S^3$ radius).

\begin{remark}[Stability: absence of ghosts]
The 601 physical DOF are ghost-free by three independent arguments: (i)~The Hodge decomposition is \emph{orthogonal} ($B \cdot C = 0$ verified to $10^{-14}$), so there are no mixed kinetic terms between gauge and physical sectors---the source of Boulware--Deser ghosts in massive gravity. (ii)~The kinetic operator $C = d_1^T d_1$ is positive semi-definite by construction: $v^T C\, v = |d_1 v|^2 \geq 0$ for all $v$, and strictly positive on the coexact subspace (since $b_1 = 0$). All 21 distinct coexact eigenvalues are positive, so the graviton propagator has no wrong-sign residues. (iii)~The decomposition $119 + 601 = 720$ is \emph{exact} by the Hodge theorem; every mode is accounted for, leaving no room for an extra ghost DOF.
\end{remark}
...
In the transverse (coexact) gauge, the linearized gravitational field equation on the 600-cell is:
\begin{equation}
\boxed{C \cdot h_T = 8\pi G \cdot T_T}
\label{eq:gravity}
\end{equation}
where $C = d_1^T d_1$ is the coexact Laplacian restricted to the physical sector, $h_T$ is the transverse (coexact) edge perturbation, and $T_T$ is the transverse stress-energy on edges. This is the discrete analog of the linearized Einstein equation in transverse-traceless gauge: $\Box\, h_{TT} = -16\pi G\, T_{TT}$.

\begin{proposition}[Ollivier Ricci-flat vacuum]
\label{prop:ricci_flat}
The Ollivier--Ricci curvature $\kappa(e)$ vanishes identically on all\/ $720$ edges of the flat $600$-cell:
\begin{equation}
...
This is not a generic property of regular graphs: the complete graph $K_n$ has $\kappa = 2/n > 0$, regular trees have $\kappa < 0$, and the Petersen graph has $\kappa = -1/2$.  The vanishing $\kappa = 0$ is specific to the 600-cell with $(d,c) = (12, 5)$.  It means the flat metric is a discrete Einstein vacuum: the linearized equation~\eqref{eq:gravity} is automatically satisfied with $h_T = 0$ when $T_T = 0$.
...
The graviton spectrum, gap-Planck identities, spectral sum rules, and the Hopf fibration connection to the Weinberg angle are developed in Supplementary Material~S7.  Key results: the coexact spectral gap $\lambda_1 = 7 - 4\phi$ has norm $N = a_1 = 5$; the gap-Planck identity $\lambda_1^{\text{exact}} \times z_{\text{Planck}} = 24 = |2T|$ ties the gravitational hierarchy to the generation group; the $b_1 = 6$ Hopf fibrations partition the 720 edges with $\sin^2\theta_W = b_1/(a_1^2+1)$.

\paragraph{Emergent graviton: polarization counting.}
The coexact spectrum of $C$ contains no eigenspace of multiplicity~9 (the symmetric traceless representation $\mathrm{Sym}^2_0$ of $H_4$), so the spin-2 graviton does not appear as a fundamental mode.  Instead, gravity \emph{emerges} from the collective spectrum.  The decisive test is the polarization ratio: at angular momentum level~$l$, the scalar Laplacian $\Delta_0$ has multiplicity $(l+1)^2$ while the coexact Laplacian $C$ has multiplicity $2l(l+2)$.  On the 600-cell, both multiplicities match the continuum $S^3$ values \emph{exactly} for $l = 1, \ldots, 5$:
\begin{equation}
...
\end{equation}
The limit $R = 2$ counts exactly two transverse polarizations per mode---the signature of the spin-2 graviton in $3\!+\!1$ dimensions.  This ratio is topological: it depends only on the multiplicity structure, not on eigenvalues or lattice spacing.  Combined with the exact TT projection ($P_{\mathrm{coexact}} = P_{TT}$), the 600-cell produces the correct graviton polarization content at every resolved angular momentum level.  The dispersion relation $\lambda_{\mathrm{coexact}}(l)/\lambda_{\mathrm{scalar}}(l)$ is approximately constant ($7.9\%$ variation over $l = 1, \ldots, 5$), confirming that tensor and scalar modes see the same effective geometry; deviations from exact $l(l+2)$ scaling are universal lattice artifacts affecting both sectors identically, consistent with a massless continuum limit (see Supplementary Material~S7.4).
...
The Hopf fibration decomposes the graph Laplacian $L = L_{\mathrm{fiber}} + L_{\mathrm{cross}}$, where $L_{\mathrm{fiber}}$ is the Laplacian of the fiber edges (each fiber a $C_{10}$ cycle, degree~2) and $L_{\mathrm{cross}}$ is the Laplacian of the cross-fiber edges (degree~10).  These two Laplacians \emph{commute}:
\begin{equation}
[L_{\mathrm{fiber}},\, L_{\mathrm{cross}}] = 0
\label{eq:fiber_cross_commute}
\end{equation}
This is a theorem, not a numerical observation: the 600-cell Cayley graph has adjacency matrix $A = \sum_{s \in S} P_s$ where $S$ is the $12$-element generating set and $P_s$ is left-multiplication by~$s$.  Since $S$ forms a single conjugacy class of~$2I$, the partition $S = S_{\mathrm{fiber}} \sqcup S_{\mathrm{cross}}$ (with $|S_{\mathrm{fiber}}| = 2$, $|S_{\mathrm{cross}}| = 10$) is stable under conjugation by any fiber generator~$g$: both elements of $S_{\mathrm{fiber}}$ lie in the abelian subgroup~$C_{10}$, so $g\,S_{\mathrm{fiber}}\,g^{-1} = S_{\mathrm{fiber}}$ and hence $g\,S_{\mathrm{cross}}\,g^{-1} = S_{\mathrm{cross}}$.  The product identity $\{fc : f \!\in\! S_{\mathrm{fiber}},\, c \!\in\! S_{\mathrm{cross}}\} = \{cf : c \!\in\! S_{\mathrm{cross}},\, f \!\in\! S_{\mathrm{fiber}}\}$ then follows from $fc = (fcf^{-1})f$ with $fcf^{-1} \in S_{\mathrm{cross}}$, yielding $A_{\mathrm{fiber}}\,A_{\mathrm{cross}} = A_{\mathrm{cross}}\,A_{\mathrm{fiber}}$ and hence~\eqref{eq:fiber_cross_commute} (verified numerically across all Hopf fibrations to machine precision).  Commutativity implies the eigenspaces decompose simultaneously: every eigenvector of $L$ is simultaneously an eigenvector of $L_{\mathrm{fiber}}$ and $L_{\mathrm{cross}}$.  The spectral gaps are:
\begin{equation}
...
\end{equation}
The Lorentzian operator admits the equivalent form $\Box = L_{\mathrm{cross}} - a_1\, L_{\mathrm{fiber}}$, which is a discrete wave equation with propagation speed
\begin{equation}
\boxed{c^2 = a_1 = 5}
\label{eq:c_squared}
\end{equation}
in lattice units, i.e.\ $c = \sqrt{a_1} = \sqrt{5} = 2\phi - 1$.  The SI value $c = 299\,792\,458\;\mathrm{m/s}$ is a unit conversion factor, not an independent constant---consistent with the standard practice of setting $c = 1$ in natural units.  The speed of light is determined by the same integer $a_1$ that fixes $\alpha$ (Eq.~\ref{eq:alpha}), $\alpha_s$, $\sin^2\theta_W$, the fermion mass spectrum, and all mixing angles.  The Hopf fiber that defines $c^2$ is the \emph{same} fiber whose holonomy $2\pi = (2a_1)\cdot(\pi/a_1)$ enters the $\alpha$ equation~\eqref{eq:alpha}, unifying the speed of light and the fine structure constant in a single geometric structure.

\paragraph{Gravitational stiffness from the Hopf anisotropy.}
Promoting edge weights $w_e > 0$ to dynamical variables (the discrete metric), the spectral action variation with respect to~$w_e$ reveals a fiber/cross anisotropy:
\begin{equation}
...
\end{cases}
\qquad
\boxed{\text{stiffness ratio} = a_1^2 = c^4 = 25}
\label{eq:gravitational_stiffness}
\end{equation}
The Hessian $\partial^2\mathrm{Tr}(\Box^2)/(\partial w_e\,\partial w_f) = 0$ for $e \ne f$: edges decouple at leading order and the flat metric is a strict minimum.  At next order, $\mathrm{Tr}(\Box^4)$ generates a non-diagonal Hessian with $101$ positive eigenvalues and $619$ zero modes, providing the kinetic term for a propagating discrete graviton consistent with spin-$2$ (the $619$ zero modes account for the $119$ gauge DOF of Section~\ref{sec:gravity} and spectral degeneracies).  The $a_1^2$-fold fiber stiffness means the temporal direction is $c^4 = 25$ times more rigid than the spatial under metric perturbations---the causal hierarchy $c^2 = a_1$ and the gravitational anisotropy share a common geometric origin in the Hopf fibration.
...
\begin{equation}
\boxed{c \;=\; \frac{N^2}{3\,N_f} + 2 \;=\; \frac{N}{h} + 2 \;=\; d_{\mathrm{ST}} + 2 \;=\; b_1}
\label{eq:variational_c}
\end{equation}
where $h = a_1 b_1 = 30$ is the Coxeter number of $E_8$, $d_{\mathrm{ST}} = a_1 - 1 = 4$ is the orbifold dimension, and the additive constant $2 = \deg(\text{fiber})$ is the fiber degree.  The operator $\Box = b_1 A_{\mathrm{fiber}} - A$ is thereby fully determined by the bootstrap condition~\eqref{eq:bootstrap}---no coefficient is chosen by hand.
\end{theorem}
...
\end{itemize}
All four values are computed in exact integer arithmetic (\texttt{int64}).  Hence $P(c) = 3N_f(c-2)$ and $P(c) = N^2$ gives $c = N^2/(3N_f) + 2$.  Substituting $N_f = N h/3$ (since the $3N_f/N = h = 30$ triangles per vertex equal the $E_8$ Coxeter number) yields $c = N/h + 2 = (a_1 - 1) + 2 = b_1$.  Each equality in~\eqref{eq:variational_c} uses a distinct structural identity: $N^2/(3N_f) = N/h$ converts face-counting to the Coxeter number; $N/h = d_{\mathrm{ST}}$ uses $|2I|/(a_1 b_1) = a_1 - 1$; and $d_{\mathrm{ST}} + 2 = b_1$ adds the fiber degree $\deg(C_{10}\text{ adjacency}) = 2$.
\end{proof}
...
\end{equation}
and satisfies $D^2 = \Delta_0 \oplus \Delta_1 \oplus \Delta_2 \oplus \Delta_3$ (the direct sum of Hodge Laplacians), with $d^2 = 0$ verified to machine precision. The Betti numbers are $(b_0, b_1, b_2, b_3) = (1, 0, 0, 1)$, confirming the $S^3$ topology.
...
\end{equation}
For the Seeley--DeWitt asymptotic expansion $S \sim \sum_k c_k\,\Lambda^{d-2k}$, the leading coefficients are:
\begin{center}
\begin{tabular}{cccl}
\toprule
$k$ & $c_k$ & $c_k / 240$ & Physical term \\
\midrule
0 & 2640 & \textbf{11} $= L_5$ (Lucas) & Cosmological \\
1 & 14880 & \textbf{62} & Einstein--Hilbert \\
2 & 55920 & \textbf{233} $= F_{13}$ (Fibonacci) & Yang--Mills \\
\bottomrule
\end{tabular}
\end{center}
All three coefficients are determined by $a_1 = 5$: $c_0 = \dim\mathcal{H}$, $c_1 = \mathrm{Tr}(D^2)$, $c_2 = \tfrac{1}{2}\mathrm{Tr}(D^4)$. The normalization $240 = 2N = |E_8\text{ roots}| = \dim(E_8) - \mathrm{rank}(E_8) = 248 - 8$ connects the 600-cell vertex count to the $E_8$ root system, while the ratios $c_k/240$ yield the Lucas number $L_5 = 11$ and the Fibonacci number $F_{13} = 233$---both deeply tied to the golden ratio.

The per-simplex traces $\mathrm{Tr}_p(D^2) / |\Delta_p|$ are $\{12, 7, 5, 4\}$ for vertices, edges, faces, and cells respectively, where 12 is the vertex degree and 5~$= a_1$ is the diagonal value of the coexact Laplacian $C$. The Einstein--Hilbert coefficient per vertex is:
\begin{equation}
c_1 / V = 14880 / 120 = 124 = \tfrac{1}{2}\dim(E_8)
\end{equation}
tying the gravitational action directly to the $E_8$ root system.
...
This spectral action provides the \textbf{discrete bosonic coefficient structure} of the framework.  By analogy with the Chamseddine--Connes theorem~\cite{chamseddine1997}, the asymptotic expansion of $\mathrm{Tr}\,f(D/\Lambda)$ takes the explicit $F_{\mu\nu}^2$-type form
\begin{equation}
...
\end{equation}
where $f_0, f_2, f_4$ are moments of the cutoff function $f$. The coefficients $(c_0, c_1, c_2) = (2640, 14880, 55920)$ contain no free parameters---they are topological invariants of the 600-cell, fully determined by $a_1 = 5$.  The gauge-channel skeleton $1 + 3 + 8 = 12$ is \textbf{derived} from the $A_5$ decomposition (Section~\ref{sec:gauge}), confirmed independently by the edge-space kernel $\ker(\Box_1) = \rho_0 \oplus 2\rho_5$ (Section~\ref{sec:edge_gauge}).
...
With $m_3$ fixed, the remaining masses follow from the mass-splitting ratio.
For the measured splittings this is a retrodictive consistency formula,
fixed only after the comparison values were public:
\begin{equation}
\frac{\Delta m^2_{21}}{\Delta m^2_{31}} = \alpha\,\phi^3 = \frac{\alpha}{2\alpha_s}\,,
\label{eq:dm_ratio}
\end{equation}
where $\alpha\,\phi^3 = 0.0309$.  The exponent $n = 3$ is not fitted: the Galois kernel Laplacian ratio is $L_8/L_1 = \phi^4$, and the conformal spectral exponent is $s = (d-1)/d = 3/4$ where $d = L_3 = 4$ is the spacetime dimension from the Lucas sequence.  Their product gives $\phi^{4 \cdot 3/4} = \phi^3$ exactly.  The factor $\alpha$ enters linearly---not as $\alpha/(4\pi)^2$---because the mass perturbation lives on the \emph{finite} spectral triple~$F$, where inner fluctuations $D_F \to D_F + A_F$ are algebraic (matrix commutators), not $4D$ loop integrals.  The coupling is $\alpha_{\text{EM}}$, not $\alpha_s$ or $\alpha_2$: among the three gauge factors, $U(1)_Y$ is the \emph{unique} one whose hypercharge distinguishes the Galois kernel partners $\rho_1$ and $\rho_8$, generating the rank-2 mixing that lifts $m_2$ from zero.  This is consistent with the Higgs mass correction $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$~\eqref{eq:higgs}, which is also $\mathcal{O}(\alpha)$ on the same finite triple.  The Galois norm $N(\phi^3) = N(1 + 2\phi) = 1 + 2 - 4 = -1$ makes $\phi^3$ a unit of~$\mathbb{Z}[\phi]$, so $|N(\alpha\phi^3)| = \alpha^2$: the splitting ratio has the arithmetic complexity of $\alpha$ alone.  Combined with a normal-hierarchy prediction $m_1 = 0$ (the neutrino mass matrix has rank~2, from the two non-trivial Galois kernel irreps), the spectrum is:
\begin{equation}
...
\begin{tabular}{lcccl}
\toprule
\textbf{Observable} & \textbf{Framework} & \textbf{Experiment} & \textbf{Error} & \textbf{Provenance} \\
\midrule
$\Delta m^2_{21}$ & $7.584 \times 10^{-5}\;\text{eV}^2$ & $(7.50 \pm 0.12)\times 10^{-5}$ (JUNO~\cite{juno2025}) & $0.70\sigma$ & retrodiction \\
$\Delta m^2_{31}$ & $2.453 \times 10^{-3}\;\text{eV}^2$ & $2.513^{+0.021}_{-0.019}\times 10^{-3}$ (NuFIT~6.0 SK~\cite{nufit6}) & $3.14\sigma$ & retrodiction \\
$\sum m_\nu$ & $58.2\;\text{meV}$ & $< 64.2\;\text{meV}$ (DESI $\Lambda$CDM, 95\% CL~\cite{desinu2025}) & pass & prediction \\
$m_\beta$ & $8.8\;\text{meV}$ & $< 450\;\text{meV}$ (KATRIN, 90\% CL~\cite{katrin2025}) & pass & prediction \\
$m_{\beta\beta}$ & $3.5\;\text{meV}$ & --- & --- & prediction \\
Mass ordering & normal & favoured & --- & prediction \\
\bottomrule
\end{tabular}
\end{center}
JUNO's first result~\cite{juno2025} was submitted on 2025-11-18, before the
February 2026 neutrino formulae.  Every JUNO comparison in the following
paragraph is therefore a consistency check, not a successful prediction.
The JUNO first result~\cite{juno2025} places the bare solar splitting at $+0.70\sigma$.  Against the official NuFIT~6.0 normal-ordering fit including SK atmospheric data~\cite{nufit6}, the bare atmospheric prediction $\Delta m^2_{31}=m_3^2$ is $-3.14\sigma$.  If the exponent receives a PMNS mixing correction $\delta = \sin^2\theta_{13} = 1/(a_1 N_{\text{eig}}) = 1/45$---the proposed leading-order effect of the $\nu_3$--$\nu_e$ mixing on the $m_3$ eigenvalue---the corrected mass is $m_3 = 2m_e/\phi^{35 - 1/45} = 50.064$~meV.  Applying this correction only to $m_3$ (Variant~I) gives $+0.70\sigma$ in $\Delta m^2_{21}$ and $-0.35\sigma$ in $\Delta m^2_{31}$; propagating it to $m_2$ by reimposing the ratio on corrected masses (Variant~II) gives $+2.07\sigma$ and $-0.35\sigma$, respectively.  The kernel-zeta derivation fixes the ratio for the bare package but does not determine the ordering of this uncomputed perturbation relative to the ratio construction.  Thus both the $1/45$ mass correction and its scope remain \textbf{pattern}; Variant~I is the eigenstate-local interpretation and is currently more consistent with the two splitting measurements, but this empirical preference is not a derivation.

\textbf{Predictive provenance.}  The measured oscillation quantities and the
point values $m_2,m_3$ reconstructed from them are \emph{retrodictions}.
Their values were known when the February 2026 sector was constructed; the
initiating prompt explicitly lists the measured splittings.  The only blind
neutrino claims are $m_1=0$, strict normal ordering,
$\sum m_\nu=58.2$--$58.9$~meV, $m_\beta=8.77$--$8.87$~meV, and the
phase-specific $m_{\beta\beta}=3.10$--$3.5$~meV.  Consistency was achieved
with values known at construction time; evidential weight rests on this blind
set and on internal derivational rigidity, not on retrodictive agreement.

\begin{remark}
The mass-splitting ratio~\eqref{eq:dm_ratio} is classified as
\textbf{derived internally but retrodictive in provenance}: its comparison
value was public before construction.  The rank-2 property $m_1=0$ is also
internally derived.  The prospective falsification claims are restricted to
the blind set: $m_1=0$, strict normal ordering,
$\sum m_\nu=58.2$--$58.9$~meV, $m_\beta=8.77$--$8.87$~meV, and the
phase-specific $m_{\beta\beta}=3.10$--$3.5$~meV.
\end{remark}
...
\midrule
$\sin^2\theta_{13}$ & $\dfrac{1}{a_1 \cdot N_{\text{eig}}} = \dfrac{1}{45}$ & 0.02222 & $0.02215^{+0.00056}_{-0.00058}$\textsuperscript{a} & \textbf{0.33\%} ($0.13\sigma$) \\[8pt]
...
%==============================================================
\section{CP Violation from Galois Theory}
\label{sec:cp}
%==============================================================
...
$SU(2)_3$ has a rank-two even Fib subcategory with based ring \(\mathbb Z[\phi]\); its full Verlinde ring has rank four.  Galois conjugation gives non-unitary Yang--Lee modular data and the formal identity \(\log(\mathcal D_{\rm Fib}/\mathcal D_{\rm YL})=\ln\phi\).  A physical dark-sector topological entropy remains \textsc{open}.  The bootstrap is the conditional pentagon matching of Theorem~\ref{thm:bootstrap}, not a dynamical selection.
...
The spectral action generates Starobinsky $R^2$ inflation with $N_e = N/2 = 60$, giving $n_s = 0.9654$ and $r = 0.003$ (within Planck~2018 bounds, testable by LiteBIRD).  The framework top mass $m_t = 172.5$~GeV places the electroweak vacuum near the stability boundary.  The scalaron mass and primordial amplitude $A_s$ depend on the test function moment $f_4$; the pattern $f_4 = N^{d_{\text{ST}}}/(2\pi)$ gives $A_s = 2.17 \times 10^{-9}$ ($+0.8\%$).  \textbf{Category: structural (NCG) $+$ pattern ($f_4$).}


%==============================================================
\section{Provenance-qualified tests}
\label{sec:predictions}
%==============================================================

Measured agreements below are retrodictions unless explicitly marked
\textbf{BLIND}.  Formula status (derived/structural/pattern) does not change
that chronology.  Only the blind set supplies prospective evidential weight.

\textbf{Tier 1: Near-term ($\sim$2027--2030).}
\begin{itemize}
\item $\sin^2\theta_{13}(\text{PMNS}) = 1/45 = 0.02222$ is a \textbf{PATTERN}, not a derived finite perturbation. NuFIT~6.0 gives $0.02215^{+0.00056}_{-0.00058}$~\cite{nufit6}. JUNO's first 59.1-day result gives $\sin^2\theta_{12}=0.3092\pm0.0087$ and $\Delta m^2_{21}=(7.50\pm0.12)\times10^{-5}\,\mathrm{eV}^2$~\cite{juno2025}.  The audited three-variant comparison empirically favors applying the $1/45$ mass correction only to $m_3$ (Variant~I), but both the correction and this scope remain \textbf{PATTERN}; \texttt{verify\_juno\_comparison.py}.
\item $\sum m_\nu = 0.058$~eV ($m_1 \approx 0$, $m_2 \approx 8.7$~meV, $m_3 = 49.5$~meV).  DESI DR2 BAO plus DR1 full shape gives $\sum m_\nu<0.0642$~eV at 95\% CL in $\Lambda$CDM~\cite{desinu2025}; the prediction currently passes but is close to this model-dependent bound.
...
\item The graviton spectral gap $7 - 4\phi$ predicts a graviton mass $\sim 10^{-34}$~eV for $R = R_{\text{Hubble}}$, far below current bounds.
\item The gap-Planck identity $\lambda_1^{\text{exact}} \cdot 4\phi^2 = 24 = |2T|$ connects gravity to the generation group.
\end{itemize}


%==============================================================
\section{Audited results, scope, and boundaries}
\label{sec:audited_consolidation}
%==============================================================

This section supersedes any stronger wording elsewhere in the historical development.  The labels are used literally: \textbf{DERIVED} denotes an exact finite theorem or an explicitly scoped numerical result; \textbf{STRUCTURAL} denotes a stated residual axiom or identification; \textbf{PATTERN} denotes a reproducible but unselected phenomenological formula; and \textbf{OPEN} denotes missing construction or analytic control.

\subsection{New exact and conditional results}

\begin{itemize}
\item \textbf{DERIVED---sampling bridges.} Evaluation of scalar harmonics with $\ell\leq2$ on the 12-point base is an injective rank-9 sampling map with exact reconstruction and an exact polynomial discrete--continuum Casimir intertwiner.  Geodesic edge integration gives a rank-16 vector intertwiner on the $\ell=1,2$ gradient and curl modes, split $8+8$ exact/coexact, and commutes with $d$.  A single local invariant edge weight cannot make all four vector sectors isometric; a mode-dependent metric is \textbf{STRUCTURAL}.  Verifiers: \texttt{verify\_low\_mode\_sampling\_intertwiner.py}, \texttt{verify\_vector\_sampling\_intertwiner.py}.
\item \textbf{DERIVED with one STRUCTURAL color axiom.} The complete $A_5$-equivariant Jacobi classification on $\mathbf{3'}\oplus\mathbf5$ contains a unique compact-simple class, $\mathfrak{su}(3)$, up to scale and equivariant isomorphism; the Killing form fixes the internal $\mathbf{3'}:\mathbf5$ normalization.  For the displayed icosahedral sampling embedding the edge metric is $20\,\mathrm{Frob}$ on $\mathbf{3'}$ and $16\,\mathrm{Frob}$ on $\mathbf5$, and its invariance excludes the split and degenerate branches.  ``Canonical'' here is conditional on identifying the actual 720-edge kernel intertwiner with that sampling embedding.  Physical selection retains the residual center-free metric-Lie-algebra axiom.  Verifier: \texttt{verify\_a5\_equivariant\_brackets.py}.
\item \textbf{DERIVED conditional---Galois doubling.} On $W\oplus\sigma(W)$ one can realize the KO-dimension-six signs $J^2=+1$, $JD=DJ$, and $J\gamma=-\gamma J$.  Galois acts as the outer automorphism on all characters.  It is \emph{not} color conjugation and does not select a finite algebra, hypercharge, or a Dirac operator.  Verifier: \texttt{verify\_galois\_doubling.py}.
\item \textbf{DERIVED---segregation theorem.} Since the unique central involution $z=-1$ commutes with every $2I$-equivariant operator, any such operator that is odd for central parity vanishes.  The odd residual subgroup types are exactly $C_1,C_3,C_5$; $C_5$ has zero Krajewski solutions.  The $C_3$ restriction admits a residual-equivariant, shared-index, Krajewski-legal block \emph{candidate}, not a constructed matrix Dirac or real spectral triple.  The subsequent first-order arena has generic gauge-plus-scale moduli dimension 122, so selection remains \textbf{OPEN}.  Verifiers: \texttt{verify\_segregation\_theorem.py}, \texttt{verify\_dirac\_selection.py}.
\item \textbf{DERIVED---K\"ahler--Dirac room.} On the oriented 2640-cell cochain complex, $\mathcal C=\bigoplus_p C^p=22\,\mathrm{Reg}(2I)$ and $2640=c_0=\operatorname{Tr}(1)$ is an identity on this same carrier.  The nonzero unbroken operator $D=d+d^*$ is odd under form parity and even under spin parity.  The carrier is available but not canonically selected as matter (the Wheeler point).  Its invariant spectrum has 52 positive levels; the lowest $D^2$ level is numerically identified as $\phi^{-4}$, not exactly algebraically certified.  The integer moments $(2640,14880,55920)$ are exact moments of this operator.  Verifiers: \texttt{verify\_kahler\_dirac.py}, \texttt{verify\_invariant\_spectrum.py}.
\item \textbf{DERIVED---ordered tower group.} The Fibonacci stationary limit has $K_0\simeq\mathbb Z[\phi]$ with the golden positive cone; the mass lattice embeds as the same abstract rooted fusion polynomial/state.  Its physical mass/scale interpretation is \textbf{STRUCTURAL}.  The exact limit follows from the unimodular bonding map; finite-level calculations are supporting evidence, not the proof.  Verifier: \texttt{verify\_bratteli\_tower.py}.
\item \textbf{DERIVED scoped dimension results.} Under the displayed criteria, fixed for rerun but not verifiably preregistered, the static complex has a calibrated counting plateau $d_N=3.0688$ and no accepted 4D plateau.  No calibrated holographic anomaly is established.  The unwarped and warped finite tower products likewise yield no $N$-stable 4D plateau at the tested sizes; the small $T^4$ control does not establish positive sensitivity, so these are scoped negative/inconclusive finite-size results.  The old vertex-index reading $d_{\mathrm{ST}}=4$ is \textbf{OPEN} and is not an index of the tested $D_4$.  A fourth dimension must be dynamical or require an analytic continuum limit.  Verifiers: \texttt{verify\_holographic\_dimension.py}, \texttt{verify\_tower\_spacetime.py}, \texttt{verify\_warped\_spacetime.py}.
\end{itemize}

\subsection{Scope and boundaries: certified matter boundary theorem}
\label{sec:scope_boundaries}

The boundary is a result, not an omission.  Three independent certificates show that Standard-Model matter is not derived from the presently canonical carriers:
\begin{enumerate}
\item the node/McKay module fails the exact representation-support and Krajewski tests;
\item the canonical oriented edge/end-point bimodule is killed by first order plus orientation oddness;
\item the smooth preprojective regular fiber is killed by its diagonal bimodule action and canonical parity.
\end{enumerate}
Thus nodes, edges, and smooth preprojective fibers do not supply the required SM matter triple within their stated canonical algebras.  Different algebras, opposite actions, non-diagonal modules, or new functors lie outside the theorem.  If an SM-shaped module exists, exact anomaly factorization forces the usual hypercharge ratios up to overall scale (and leaves the extra singlet neutrality as an additional condition); this is the precise M15/M16 result.  A real SM generation has trace-index ratio $5:3:3$, not the old $8:5:2$ pattern.  Consequently the gauge prefactors $(8/15,1/3,2/15)$ remain \textbf{PATTERN}: no common derived matter trace fixes them.  Verifiers: \texttt{verify\_bimodule\_krajewski.py}, \texttt{verify\_edge\_matter.py}, \texttt{verify\_preprojective\_matter.py}, \texttt{verify\_matter\_functor.py}, \texttt{verify\_matter\_module.py}, and \texttt{verify\_matter\_trace\_indices.py}.

Likewise, the identity producing $4a_1\phi^4$ is \textbf{DERIVED} as finite spectral arithmetic, but its identification with the physical electromagnetic inverse coupling is \textbf{STRUCTURAL}.  The missing axiom is a normalized discrete-to-continuum $U(1)$ cochain map that is an isometry for specified inner products together with a fixed charge unit; \texttt{verify\_coupling\_constants.py}.

\subsection{Audited master status table}

\begin{table}[H]
\centering\scriptsize
\begin{tabular}{p{0.36\linewidth}p{0.13\linewidth}p{0.43\linewidth}}
\toprule
\textbf{Claim} & \textbf{Status} & \textbf{Verifier(s)}\\
\midrule
Scalar $\ell\le2$ and rank-16 vector sampling intertwiners & DERIVED & \texttt{verify\_low\_mode\_sampling\_intertwiner.py}; \texttt{verify\_vector\_sampling\_intertwiner.py}\\
$A_5$ bracket classification; compact color under displayed metric embedding & DERIVED / STRUCTURAL & \texttt{verify\_a5\_equivariant\_brackets.py}\\
KO6 Galois double; Galois outer; not color conjugation & DERIVED conditional & \texttt{verify\_galois\_doubling.py}\\
Central segregation and subgroup escape classification & DERIVED & \texttt{verify\_segregation\_theorem.py}\\
$C_3$ legal block candidate; 122-dimensional selection moduli & DERIVED candidate / OPEN & \texttt{verify\_dirac\_selection.py}\\
Matter boundary theorem; anomaly-forced hypercharge if a module exists & DERIVED scoped / OPEN module & six matter verifiers listed above\\
$22\,\mathrm{Reg}(2I)$ K\"ahler--Dirac carrier and exact moments & DERIVED & \texttt{verify\_kahler\_dirac.py}; \texttt{verify\_invariant\_spectrum.py}\\
52 levels; gap $\phi^{-4}$ & DERIVED count / numerical identification & \texttt{verify\_invariant\_spectrum.py}\\
$K_0$ Fibonacci tower and golden order & DERIVED & \texttt{verify\_bratteli\_tower.py}\\
Static/tower/warped dimension-four claims & scoped negative; $d_{\mathrm{ST}}=4$ OPEN & three dimension verifiers listed above\\
Gauge prefactors $8:5:2$ & PATTERN (no-go) & \texttt{verify\_matter\_trace\_indices.py}\\
$4a_1\phi^4$ as physical $1/\alpha_0$ & STRUCTURAL & \texttt{verify\_coupling\_constants.py}\\
$1/45$ neutrino correction and Variant-I scope & PATTERN; empirically favored & \texttt{verify\_juno\_comparison.py}\\
\bottomrule
\end{tabular}
\caption{Binding post-audit status ledger.  Exactness claims apply only at the scope stated in the corresponding verifier and result note.}
\label{tab:audited_status}
\end{table}
...
To guard against post-hoc numerological fitting, we distinguish three evidential levels: (i)~\textbf{Rigorous derivation} ($a_1 = 5 \to$ result via established mathematics, no choices): gauge group, $N_{\text{gen}}$, the constructive $(a,b)$ assignment, exact discrete scalar response, the simplicial Yang--Mills stiffness $K_{\mathrm{YM}} = b_1$, CKM bare exponents, $\delta_{\text{CKM}}$, $\theta_{\text{QCD}} = 0$.  (ii)~\textbf{Algebraically motivated reproduction} (framework constants only, but with a well-motivated identification step): mass corrections ($\chi^2/\text{dof} = 0.28$, $p = 0.97$), coupling constants (all three discrete ingredients of the $\alpha$ tree-level coefficient derived, with only the KK identification and the $M_Z$-scale matching still structural), and continuum bosonic analogies from the discrete spectral action.  (iii)~\textbf{Open problems}: continuum interpretation of the discrete $\alpha$ coefficient, $M_Z$ scale, cosmological constant mechanism, nonlinear gravity.  Table~\ref{tab:summary} categorizes every result:
...
$m_Z$ ($n\!=\!a_1^2$ selected by RG bootstrap, Eq.~\ref{eq:rg_cost}), $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$, $m_e/m_P = \alpha^{4\phi^2}$ & 0.008--0.24\% & \textbf{Structural} \\
\midrule
...
Mass corrections: $\alpha = 1 - d_{\text{eff}}/d_{\text{ST}}$ (Eq.~\ref{eq:morrey_alpha}) & Exact & \textbf{Derived}$^\dagger$ \\
\midrule
...
Gauge prefactors $(8/15,1/3,2/15)$ & --- & \textbf{Pattern (no-go)}$^\S$ \\
Emergent graviton: $R(l) \to 2$ polarizations (Eq.~\ref{eq:polarization_ratio}) & Exact & \textbf{Derived} \\
\midrule
...
\bottomrule
\end{tabular}
\caption[Summary of predictions.]{Legacy summary, subject to the binding audited ledger in Table~\ref{tab:audited_status}.  ``Derived'' = deduced at the explicitly stated finite scope; ``Structural'' = requires a named identification axiom; ``Pattern'' = reproducible formula without a selecting derivation.  $\dagger$The use of $d_{\mathrm{ST}}=4$ in the mass-correction ansatz is structural; the static spectral derivation is open.  $\ddagger$The scalar-response theorem is exact on the discrete 600-cell; the continuum PPN interpretation is not claimed.  $\S$The vertex census does not fix a common normalized matter trace: a real SM generation yields $5:3:3$, while $8:5:2$ remains a pattern.}
\label{tab:summary}
\end{table}
...
\begin{enumerate}
\item \textbf{Mass scale.}  The electron is the unique internally selected anchor state: it is the Galois fixed point ($z_e = 0$), the McKay graph origin (trivial irrep $\rho_0$), and the ground state of the mass ladder ($n = 0$).  The physically meaningful dimensionless hierarchy $m_e/m_P = \alpha^{4\phi^2}$ is derived from the spectral data ($0.24\%$ agreement); what remains external is only the conversion from Planck units to laboratory units.  This is a genuine no-go: the spectral data are pure numbers, so by dimensional analysis they cannot generate a quantity with units of GeV.  But it is a much weaker gap than ``$m_e$ is a free parameter'': the framework determines \emph{all} dimensionless ratios, and the sole residual freedom is a unit convention, not a physical parameter.  In particular, $m_e$ fixes the unit system, not the physics: every prediction in this paper---coupling constants, mass ratios, mixing angles, CP phases---is a pure number derived from $a_1 = 5$ with zero adjustable parameters.  No measurement of $m_e$ (or any other dimensionful quantity) can falsify or confirm these dimensionless predictions.
\item \textbf{Effective action.} The vacuum-selection functional $S_{\mathrm{eff}} = S_{\mathrm{EW}} + w_c S_{\mathrm{cosmo}}$ is robust and sharply minimized at $(a_1,n) = (5,25)$ on the scanned domain.  Its \emph{local} form around the minimum is structurally justified: the linear term vanishes identically, the curvature~\eqref{eq:curvature} is derived from spectral data, and the quadratic approximation reproduces the exact cost to ${\sim}\,10\%$ at $|\delta n| = 1$.  The \emph{global} form need not be specified: the selection is universal within the class of non-negative mismatch functionals with one-loop RG monotonicity ($L^2$, $L^1$, $L^\infty$, and Kullback--Leibler norms all select the same vacuum, verified numerically).  The sole remaining open problem is the absolute scale conversion (Section~\ref{sec:vacuum_selection}).
\item \textbf{Spectral action normalization.} The exact discrete moments $(c_0,c_1,c_2) = (2640,14880,55920)$ belong to the explicitly constructed K\"ahler--Dirac operator $D=d+d^*$.  The gauge prefactors $(8/15,1/3,2/15)$ are \textbf{PATTERN}, not derived or structural: the available symmetry permits independent invariant quadratic normalizations and no common derived matter representation and trace fixes them.  A real SM generation instead gives $5:3:3$.  What remains open is a controlled continuum identification with the full bosonic Standard Model action.
\item \textbf{Gravity and dimension.} The exact discrete scalar-response identity $\gamma_{\mathrm{disc}} = 1$ and the stated polarization count are finite results.  The calibrated static spectrum is approximately three-dimensional ($d_N=3.0688$), and neither tested unwarped nor warped finite tower product has an $N$-stable 4D plateau under the criteria fixed for rerun.  The vertex-index claim $d_{\mathrm{ST}}=4$ is explicitly \textbf{OPEN}; the fourth direction must be dynamical or established through analytic continuum limits.
...
\item \textbf{Mass correction unification.} The three cascade levels are identified as Morrey--Sobolev regimes at effective dimensions $d_{\text{eff}} = 0, 1, 4$ (Eq.~\ref{eq:morrey_alpha}).  The spectral action provides the $p = 4$ energy and the Galois anti-symmetric fluctuation generates the corrections.  The remaining gaps are: (i)~$d_{\text{eff}} = d_{\text{ST}}$ for non-units rests on a structural argument (spinorial doubling per archimedean place), not a rigorous proof; (ii)~the Level~2$\to$Level~3 boundary is discontinuous ($+0.163$ vs.\ $-0.400$), reflecting different $d_{\text{eff}}$ regimes; (iii)~the explicit spectral action computation of the Morrey normalization coefficients has not been carried out.
\end{enumerate}

\subsection{Statistical Robustness}
\label{sec:statistics}

Internal coherence and assignment uniqueness may constrain the construction,
but they do not remove the provenance look-elsewhere risk: the experimental
targets were known.  No post-hoc probability such as the former
$P_9\lesssim10^{-12}$ is claimed.  A transparent illustrative expression
census and its limitations are recorded in
\texttt{prediction\_provenance\_ledger.md}.  Prospective evidential weight is
reserved for the ledger's blind claims.
...
\end{enumerate}
The cost function~\eqref{eq:seff} uses no external input: the one-loop beta coefficients $b_i = (41/10,\,-19/6,\,-7)$ are themselves derived from the framework's particle content ($N_{\text{gen}} = 3$ and the gauge group), so the RG running is intrinsic.  The exponent $n_0 = a_1^2 = 25$ is a spectral invariant---the multiplicity of the zero eigenvalue of the adjacency matrix (Table~\ref{tab:spectrum})---not an external choice.  The local form of the cost around $n_0$ is structurally constrained:
...
What remains open is the absolute scale conversion---why the lattice spacing is $1/(m_e\,\phi^{25})$ rather than, say, $1/m_P$---which is the hierarchy problem in discrete form.  The 600-cell spectral data are pure numbers in lattice units; no functional of these data can produce a dimensionful quantity without at least one dimensionful input.  Given one such input ($m_e$ or equivalently $m_P$), the spectrum determines all other scales through derived ratios ($m_e/m_P = \alpha^{4\phi^2}$ to $0.24\%$, $m_Z/m_e = \phi^{a_1^2} \cdot R$ to $0.09\%$).  The hierarchy $m_e/m_P \sim 10^{-22}$ is $\alpha^{10.5}$, where both $1/137$ and $10.5$ are moderate numbers derived from~$a_1$; the large ratio arises from exponentiation, not fine-tuning.
...
The framework has a \emph{two-layer architecture}.  The first layer is the Fibonacci based ring and a conditional pentagon matching that singles out \(a_1=5\).  The equality \(N_{\rm gen}=a_1-2=k=3\) is not a categorical derivation: the integers count different objects and a generation functor remains \textsc{open}.  The ordered \(\mathbb Z[\phi]\) tower is \textsc{derived}, while its mass interpretation is \textsc{structural}.  The exact conformal embedding \(G_2{}_1\times F_4{}_1\subset E_8{}_1\) supplies a new \textsc{structural} bridge to the second pillar, but no functor yet connects it to the 600-cell operators or matter module.

The key structural results, grouped by theme, are:

\begin{enumerate}
\item \textbf{Gauge group and generations.}  The permutation representation on the icosahedral vertex figure gives $12 = \mathbf{1} \oplus \mathbf{3} \oplus \mathbf{3'} \oplus \mathbf{5}$; Theorem~\ref{thm:gauge_id} proves $G = U(1) \times SU(2) \times SU(3)$ is unique.  The Generation Theorem gives $N_{\text{gen}} = 3$ from Galois invariance on $\mathbb{Z}[\phi]$.

\item \textbf{Coupling constants and gravity.}  Two gauge couplings are algebraically derived ($\alpha_s = 1/(2\phi^3)$ from the $\mathbb{Z}[\phi]$ factor structure (0.11\%), $\sin^2\theta_W = 6/26$ from graph combinatorics (0.19\%)); the third ($\alpha$ from the icosahedral Laplacian, 0.0001\%) is now better constrained than before: the tree-level coefficient admits the exact discrete form $1/\alpha_0 = N/(K_{\mathrm{YM}}\lambda_1(L_{\mathrm{fiber}})^2)$, where $N=|2I|$ is combinatorial, $K_{\mathrm{YM}} = b_1$ is the exact simplicial Yang--Mills stiffness, and $\lambda_1(L_{\mathrm{fiber}})=2-\phi$ is the Hopf-fiber spectral gap.  Thus all three factors of the linear coefficient are internally derived; the remaining structural step is the Kaluza--Klein identification of this discrete tree-level coefficient with the electromagnetic inverse coupling.  The $\alpha$ equation $\text{Vol}(S^1)\,\alpha^2 - (N/b_1)\phi^4\,\alpha + 1 = 0$ has three coefficients of distinct origin: topological (Hopf holonomy $2\pi$), gauge-theoretic (exact Yang--Mills stiffness $K_{\mathrm{YM}} = b_1$), and spectral (equivalently $\phi^4 = \lambda_1(L_{\mathrm{fiber}})^{-2}$), plus normalization ($1$).  The Vieta product $\alpha\alpha' = 1/\text{Vol}(S^1)$ is the standard Kaluza--Klein coupling on the Hopf fiber.  The hierarchy identity $m_e/m_P = \alpha^{1/\alpha_s+2}$ connects all three forces.  On the bosonic side, the exact discrete spectral-action core is now the coefficient triple $(2640,14880,55920)$ with reduced triple $(11,62,233)$ and identity~\eqref{eq:seeley_diophantine}, together with the exact discrete scalar-response theorem $\gamma_{\mathrm{disc}} = 1$ and the exact abelian Yang--Mills response $S_{\mathrm{YM}}(\theta)=2400\sin^2(\theta/20)$; the gauge prefactors $(8/15, 1/3, 2/15)$ are now explained structurally by the vertex-type decomposition $120 = 8(Q_8) + 16(2T) + 96$ under the subgroup chain $Q_8 \subset 2T \subset 2I$ (Eq.~\ref{eq:gauge_prefactors}); what remains open is the continuum identification.

\item \textbf{Fermion masses and uniqueness.}  The displayed mass formula gives the reported numerical fit.  Its ``Verlinde radiative correction'' is \textsc{pattern}: neither the \(S\)-matrix normalization nor the fusion-matrix norm supplies a categorical-to-QED map.  The separately audited arithmetic assignment retains the status recorded in the master ledger.
...
\item \textbf{Galois kernel (Theorem~\ref{thm:galois_kernel}).}  The Lorentzian operator $\Box = b_1 A_{\text{fiber}} - A$ on any Hopf fibration has $\ker(\Box) = \rho_0 \oplus \rho_1 \oplus \rho_8$ with $\dim = 9 = N_{\text{eig}}$.  The coefficient $b_1$ is the \emph{unique} nontrivial integer giving a nonzero kernel (see Supplementary Material~S7.2), and the kernel selects the smallest irrep from each Galois orbit, with dimension sum $1 + 2 + 2 = 5 = a_1$.  The kernel is stable across all 27 Hopf fibrations.
\end{enumerate}
...
The electron is the unique internally selected anchor state---the Galois fixed point of the mass lattice, the trivial irrep on the McKay graph, and the ground state of the mass operator.  In Planck units, its mass is a derived pure number: $m_e/m_P = \alpha^{4\phi^2}$ ($0.24\%$).  The physically meaningful dimensionless hierarchy is therefore explained; what remains external is only the conversion from Planck units to laboratory units.  Whether even that residual convention can be eliminated---for example, by coupling the discrete geometry to a gravitational path integral that generates a Planck mass dynamically---remains open.
...
All numerical results reported in this paper can be reproduced using the self-contained Python scripts available at \url{https://github.com/razvananghelina/One-Integer-Three-Generations}. The repository contains five verification scripts covering: coupling constants, 600-cell Laplacian spectrum and irrep localization, fermion masses and mixing angles, Berry phase computation, and spectral action properties. Each script requires only NumPy and SciPy.
...
\bibitem{juno2025}
JUNO Collaboration, ``First measurement of reactor neutrino oscillations at JUNO,'' arXiv:2511.14593 [hep-ex] (2025).

\bibitem{nufit6}
I. Esteban, M. C. Gonzalez-Garcia, M. Maltoni, I. Martinez-Soler, and T. Schwetz, ``NuFIT~6.0,'' arXiv:2410.05380 [hep-ph] (2024).

\bibitem{desinu2025}
DESI Collaboration, ``Constraints on Neutrino Physics from DESI DR2 BAO and DR1 Full Shape,'' arXiv:2503.14744 [astro-ph.CO] (2025).

\bibitem{katrin2025}
KATRIN Collaboration, ``Direct neutrino-mass measurement based on 259 days of KATRIN data,'' arXiv:2406.13516 [nucl-ex], \textit{Science} \textbf{388}, 180--185 (2025).
