...
\begin{abstract}
We show that many exact finite invariants and candidate Standard-Model
structures are traced in this framework to a single integer $a_1=5$, the
unique positive solution of $a_1!=4a_1(a_1+1)$.  The status ledger below
separates those theorems from physical identifications and numerical
patterns.  In particular, $m_e/m_P=\alpha^{4\phi^2}$ is a close dimensionless
\textsc{pattern}, not a derived Planck normalization: the present spectral
data contain no selected absolute gravitational scale.
...
\end{tabular}
\end{center}
Only $k = 0, 1, 3$ satisfy (i)--(ii); condition (iii) eliminates $k=0$; among $k=1$ and $k=3$, the latter has the \emph{minimal} integer prefactor ($c = 2$ vs.\ $c = 8$), selecting it uniquely.  The identities $\text{Tr}(\phi^3)=4$ and $k=3=N_{\text{gen}}$ are exact arithmetic facts.  The former is not a spacetime-dimension measurement: the verified carrier is three-dimensional and no fourth direction is derived.  The norm follows as a consequence:
\begin{equation}
...
\textbf{Physical interpretation.} The trace condition ties $\alpha_s$ to the gauge sector ($\text{Tr} = \dim(SU(3)) = \text{rank}(E_8)$).  The factor $c=2=\text{rank}(E_8)/\text{Tr}(\phi^3)$ is an exact algebraic ratio, not a ratio to a derived spacetime dimension.  The negative norm $N=-4$ makes the Galois conjugate $z'$ negative; reading that sign as asymptotic freedom remains the stated physical dictionary.

\textbf{Hierarchy-pattern identity.}  The exact spectral weights $z=2\phi^3$ and $z_P=4\phi^2$ are related by
\begin{equation}
\boxed{z_P = \frac{1}{\alpha_s} + 2}
\label{eq:hierarchy_identity}
\end{equation}
since $4\phi^2=2\phi^3+2$.  In $\mathbb Z[\phi]$, the two quantities share the same irrational part.  Reading $z_P$ as a Planck exponent and hence $m_e/m_P=\alpha^{1/\alpha_s+2}$ is \textsc{pattern/open}.
...
\textbf{Arithmetic normalization of the coefficient.} The displayed lepton coefficient uses the exact trace value
\begin{equation}
\boxed{c_\ell = \frac{C \cdot \phi^3}{\mathrm{Tr}(\phi^3)} = \frac{C \phi^3}{4}}
\label{eq:cell}
\end{equation}
where $\mathrm{Tr}(a+b\phi)=2a+b$ is the $\mathbb Z[\phi]$ trace.  The Lucas trace $\mathrm{Tr}(\phi^3)=4$ singles out $n=3$ arithmetically, but it does not establish a four-dimensional Sobolev space.  Consequently the choices $k=3/4$ and $c_\ell=C\phi^3/4$ remain part of the registered mass-correction \textsc{pattern}; they are not derived from spacetime dimension.
...
\textbf{Physical interpretation.} The trace and norm conditions select the algebraic number $z=4\phi^2$ once those two conditions are imposed.  Their values, $12$ and $16$, coincide with other finite invariants of the 600-cell.  They do not derive a four-dimensional spacetime or identify $z$ with a gravitational coupling; that dictionary is \textsc{pattern/open}.

Conditionally identifying the numerical hierarchy with the Planck hierarchy gives
\begin{equation}
...
\end{equation}
an exact arithmetic coincidence with $|2T|$.  It is not a derivation of the gravitational coupling strength.

\textbf{Remark.}  Setting $m_P=1$ is a unit convention and does not derive which finite spectral scale is the physical Planck scale.  The numerical relation $m_e/m_P=\alpha^{4\phi^2}$ is retained as a \textsc{pattern}; the absolute gravitational normalization remains open.
...
Numerically, the hierarchy pattern rewrites the large ratio as
$(1/\alpha)^{4\phi^2}\approx137^{10.5}$.  The exponent and its Galois norm are
exact algebraic data, but the rule that maps them to $m_e/m_P$ is not selected
by the finite action.  Therefore this observation does not solve the hierarchy
problem until a gravitational scale and coupling are independently derived.

\subsection{The Spectral Weight and the Candidate Electromagnetic--Gravitational Link}
\label{sec:spectral_weight}

For each irrep $\rho_k$ of $2I$ with dimension $d_k$ and character $\chi_k$ at the nearest-neighbor class, define the exact spectral weight $z_k=12\chi_k^2/d_k$.  The proposed Planck exponent is the weight of $R_3$:
\begin{equation}
\boxed{z_{\text{Planck}} = z_{R_3} = \frac{12\,\phi^2}{3} = 4\phi^2}
\label{eq:zP_spectral}
\end{equation}
The linear coefficient of the $\alpha$ equation factorizes exactly as
$B=4a_1\phi^4=a_1\phi^2z_{R_3}$.  The additional identities for
$z_{\mathrm{mass}}$ and the total weight are likewise algebraic.  Calling
$z_{R_3}$ a Planck exponent is the unproved physical step; the factorization
alone does not supply gravitational units or dynamics.
...
This section records exact finite Hodge and response identities and audits their possible gravitational interpretation.  Neither a Planck mass nor the Einstein field equation has been derived from the finite data.
...
branch.  At fixed $\det G=1$, if $a,b,c>0$ are the eigenvalues of $G$, Schur's
inequality and AM--GM give $R(G)/2=4(ab+bc+ca)-(a+b+c)^2\leq3$, with equality
only at $a=b=c=1$.  Hence the round metric is the unique global minimizer of
the ordinary $A_2=-(2/3)\int R\,dV$ coefficient among all fixed-volume
left-invariant metrics on $SU(2)$.  This extends the local Hopf Hessian to a
global homogeneous shape-selection result.  The two endpoints can be compared
with one coefficient: Cheeger's skeleton expansion and the exact Hodge cone
terms of Fursaev--Miele~\cite{cheeger1983,fursaevmiele1997} give, at equal volume,
$A_2(g_0)=-78.9568352087\ldots<A_2(g_R)=-78.8719985927\ldots$.
The exact conical correction is essential: retaining only the linear Regge
deficit term reverses this ordering.  Thus the same conditional coefficient
prefers the round endpoint.  On the preregistered affine interpolation
$(1-u)g_R+ug_0$, the de Rham transmittal face term is nonzero with an exact
$u(1-u)$ factor.  After including it, all $200$ interior points of the frozen
grid, at five converged quadrature orders, lie strictly above the round
endpoint and decrease toward it.  This is a numerical pattern, not a proof on
the continuous interval, and it is not yet a complete metric action.
Nor does the finite ratio $c_1/(2c_0)=31/11$ fix a physical scale: under
$g\mapsto c^2g$ it scales
as $c^{-2}$, and the covariant prescription $t=\alpha/[c_1/(2c_0)]$ retains an
undetermined dimensionless $\alpha$.
...
The spectral weight itself is exact.  Identifying it with a Planck exponent is
an additional physical assignment, so the hierarchy formula remains
\textsc{pattern/open} rather than a derived gravitational scale.
...
\label{eq:dm_ratio}
\end{equation}
where $\alpha\phi^3=0.0309$.  The factorization
$\phi^3=(\phi^4)^{3/4}$ is exact, but the exponent $3/4$ previously came from
the rejected four-dimensional reading.  No current spectral dimension or
finite summability theorem selects it.  The ratio is therefore a
retrodictive \textsc{pattern}, even though its algebraic ingredients are exact.
Combined with the separately proposed normal hierarchy, it gives:
\begin{equation}
...
\begin{enumerate}
\item \textbf{Mass scale.}  The electron is an internally distinguished algebraic anchor state: it is the Galois fixed point, the trivial McKay irrep and the origin of the proposed mass ladder.  The relation $m_e/m_P=\alpha^{4\phi^2}$ agrees numerically at the stated level but remains \textbf{PATTERN/OPEN}: neither the finite spectral action nor the new smooth shape selector fixes a Planck scale, Newton constant or the map from the exact spectral weight $4\phi^2$ to this physical hierarchy.
...
What remains open is both the absolute scale conversion and the physical rule
mapping the exact spectral weight $4\phi^2$ to $m_e/m_P$.  The numerical
relation $m_e/m_P=\alpha^{4\phi^2}$ is a close pattern, not a derived ratio.
The finite spectral data alone are dimensionless and do not supply a Newton or
...
Planck-hierarchy formula remains a numerical pattern because the theory has
not selected an absolute gravitational scale.  A dynamical metric action and
Newton normalization would be required before it could become a derivation.
...
\bibitem{cheeger1983}
J. Cheeger, ``Spectral geometry of singular Riemannian spaces,'' \textit{J. Differential Geom.} \textbf{18}, 575--657 (1983).

\bibitem{fursaevmiele1997}
D. V. Fursaev and G. Miele, ``Cones, spins and heat kernels,'' \textit{Nucl. Phys. B} \textbf{484}, 697--723 (1997).
