...
\end{equation}
This coefficient links the Weinberg angle to the number of generations: algebraically $C=\tfrac23\sin^2\theta_W=4/(a_1^2+1)$.  The numerator is the exact integer $a_1-1=4$; interpreting it as a spacetime dimension is \textbf{REJECTED} for the fixed carrier.  The verified cochain geometry is three-dimensional, and a fourth dynamical/bulk direction remains \textbf{OPEN} (Section~\ref{sec:audited_consolidation}).

\textbf{The correction formula.}  The correction mechanism depends on the algebraic nature of each fermion in $\mathbb{Z}[\phi]$: up quarks ($T_3 = +\tfrac{1}{2}$) couple positively to the algebraic norm $|N(z)|$, down quarks ($T_3 = -\tfrac{1}{2}$) couple negatively and are suppressed by $|\phi'/\phi| = 1/\phi$ (the Galois conjugate ratio), and leptons use the fitted structural exponent $k=3/4$.  The former identification $k=1-1/d_{\mathrm{ST}}$ is not derived by the finite operator.  The corrected mass is $m_f = m_e \cdot \phi^{n_f + \delta_f}$, where:
\begin{equation}
...
\end{equation}
The exponent $k=3/4$ is a reproducible input pattern:
\begin{equation}
\boxed{k=\frac34}
\label{eq:lepton_k}
\end{equation}
The former three-step argument does not derive this exponent:

\emph{Step 1: conditional Morrey identity.}  Morrey's inequality gives exponent $1-1/p$ for $W^{1,p}(\mathbb R)$.  It yields $3/4$ only after assuming $p=4$.

\emph{Step 2: finite summability does not fix $p$.}  On a finite-dimensional Hilbert space every inverse on the nonzero sector belongs to every Schatten class.  Hence the verified finite triple has no summability exponent selecting $p=4$.

\emph{Step 3: saturation remains unproved.}  The vanishing of the displayed lepton eigenvalue does not force a Morrey extremal or its normalization.  The power law remains a phenomenological pattern pending a selecting variational principle.

Thus no spacetime dimension is inferred from the successful numerical exponent.
...
\textbf{Conditional Sobolev--Arakelov pattern.}  The following assignment is a compact parametrization of the three functional forms, not a dimension derivation.  Define the labels
\begin{equation}
d_{\text{eff}} =
\begin{cases}
...
\label{eq:d_eff}
\end{equation}
Conditionally choosing $p=4$ gives the arithmetic pattern:
\begin{equation}
...
\end{equation}
The exact identity $\mathrm{Tr}_{\mathbb Q(\sqrt5)/\mathbb Q}(\phi^3)=4$ fixes the numerical denominator $\phi^3/4$, but does not turn the fourth matrix moment into a four-dimensional Sobolev energy.  The physical unification is therefore \textbf{PATTERN/OPEN}.  See Supplementary Material~S5.10 for the corrected gap analysis.
...
\end{equation}
where $h = a_1 b_1 = 30$ is the Coxeter number of $E_8$ and the additive constant $2 = \deg(\text{fiber})$ is the fiber degree.  The integer $a_1-1=4$ also equals the real dimension of the ambient orbifold $\mathbb C^2/2I$, but this arithmetic equality does not make it the dimension of the boundary cochain operator.  The operator $\Box = b_1 A_{\mathrm{fiber}} - A$ is thereby fully determined by the bootstrap condition~\eqref{eq:bootstrap}---no coefficient is chosen by hand.
\end{theorem}
...
\end{itemize}
All four values are computed in exact integer arithmetic (\texttt{int64}).  Hence $P(c) = 3N_f(c-2)$ and $P(c) = N^2$ gives $c = N^2/(3N_f) + 2$.  Substituting $N_f = N h/3$ yields $c = N/h + 2 = (a_1 - 1) + 2 = b_1$.  These are combinatorial identities; no spacetime dimension is used in the proof.
\end{proof}
...
\end{equation}
and satisfies $D^2 = \Delta_0 \oplus \Delta_1 \oplus \Delta_2 \oplus \Delta_3$ (the direct sum of Hodge Laplacians), with $d^2 = 0$ verified exactly over the integer incidence matrices. The Betti numbers are $(b_0, b_1, b_2, b_3) = (1, 0, 0, 1)$, consistent with the independently known fact that this complex is the boundary $S^3$ of the convex 600-cell.  The Betti numbers alone would identify only its homology profile.

The finite \textbf{spectral action} \cite{chamseddine1997} is:
\begin{equation}
\boxed{S = \mathrm{Tr}\,f(D/\Lambda)}
\label{eq:spectral_action}
\end{equation}
On this finite Hilbert space the heat trace has the Taylor expansion
\begin{equation}
\operatorname{Tr}e^{-tD^2}=c_0-c_1t+c_2t^2+O(t^3).
\end{equation}
The first three finite moments are:
\begin{center}
\begin{tabular}{cccl}
\toprule
$k$ & $c_k$ & $c_k / 240$ & Finite definition \\
\midrule
0 & 2640 & \textbf{11} $= L_5$ (Lucas) & $\operatorname{Tr}I$ \\
1 & 14880 & \textbf{62} & $\operatorname{Tr}D^2$ \\
2 & 55920 & \textbf{233} $= F_{13}$ (Fibonacci) & $\tfrac12\operatorname{Tr}D^4$ \\
\bottomrule
\end{tabular}
\end{center}
All three moments are determined by $a_1 = 5$.  They are \emph{not}
Seeley--DeWitt coefficients: a finite heat trace tends to $2640$ as $t\to0$
and has no negative-power exponent from which a dimension can be read.  A
continuum Seeley--DeWitt interpretation requires a controlled refinement
asymptotic, which is currently open.  The normalization $240 = 2N = |E_8\text{ roots}| = \dim(E_8) - \mathrm{rank}(E_8) = 248 - 8$ connects the 600-cell vertex count to the $E_8$ root system, while the ratios $c_k/240$ yield the Lucas number $L_5 = 11$ and the Fibonacci number $F_{13} = 233$.

The per-simplex traces $\mathrm{Tr}_p(D^2) / |\Delta_p|$ are $\{12, 7, 5, 4\}$ for vertices, edges, faces, and cells respectively, where 12 is the vertex degree and 5~$= a_1$ is the diagonal value of the coexact Laplacian $C$. The second finite moment per vertex is:
\begin{equation}
c_1 / V = 14880 / 120 = 124 = \tfrac{1}{2}\dim(E_8)
\end{equation}
an exact arithmetic relation to the $E_8$ dimension.  Identifying it with an
Einstein--Hilbert coefficient is a separate, presently open continuum step.
...
following Chamseddine--Connes expression is only a continuum template; it is
not an asymptotic expansion derived from this fixed finite matrix:
\begin{equation}
...
\end{equation}
where $f_0, f_2, f_4$ are moments of the cutoff function $f$. The numbers $(c_0, c_1, c_2) = (2640, 14880, 55920)$ contain no free parameters, but they are triangulation- and operator-dependent spectral moments, not topological invariants or derived continuum couplings.  The gauge-channel skeleton $1 + 3 + 8 = 12$ is \textbf{derived} from the $A_5$ decomposition (Section~\ref{sec:gauge}), confirmed independently by the edge-space kernel $\ker(\Box_1) = \rho_0 \oplus 2\rho_5$ (Section~\ref{sec:edge_gauge}).
...
\end{equation}
where $31 = a_1^2 + b_1$ and $11 = a_1 + b_1 = L_5$ (Lucas number) are framework constants.  Its interpretation as part of a continuum spectral-action parameter remains open.

\begin{proposition}[Finite-moment Diophantine identity]\label{prop:seeley_diophantine}
...
The Cayley graph approximates $S^3$ with bi-Lipschitz distortion $b_1/a_1 = 6/5$ and Latr\'emoli\`ere propinquity bound $\Lambda^* \leq \pi/(a_1\phi) \approx 0.386$ (see Supplementary Material~S7.3).  The graph Laplacian Green's function $G(x,y) = \sum_{\lambda_k > 0} \psi_k(x)\psi_k(y)/\lambda_k$ matches the $S^3$ continuum Green's function in normalized shape ($G(d)/G(1)$ at graph distance~$d$) with RMS error $0.6\%$ over $d = 1, \ldots, 4$.  Newton's $1/r$ potential does not appear directly.  Deriving a $1/r$ regime and an Einstein--Hilbert coefficient requires a controlled continuum limit and is open; it does not follow from the finite moment $c_1=14880$ alone.
...
Conditionally inserting the finite data into a four-dimensional continuum NCG template gives the Starobinsky pattern $N_e=N/2=60$, $n_s=0.9654$ and $r=0.003$.  No four-dimensional continuum spectral action has been derived from the fixed matrix, so this is not an output of the certified finite moments.  The scalaron amplitude additionally uses the pattern $f_4=N^4/(2\pi)$.  \textbf{Category: pattern/open.}
...
\item \textbf{DERIVED scoped dimension results.} The static complex has a target-conditioned counting interval $d_N=3.0688$ and no accepted 4D interval.  Independently, the complete K\"ahler--Dirac heat flow has global maximum $3.295664$ and therefore never reaches four; its target-free 2\% shoulder near $3.27$ is shorter than the registered half-decade plateau gate.  The old $9-13+1-1=-4$ is an alternating nullity of non-intertwining $\Box_p$ operators and its spacetime reading is \textbf{REJECTED}.  The unwarped and warped finite products likewise have no stable 4D plateau.  A fourth bulk/time direction remains \textbf{OPEN}, not derived.  Verifiers: \texttt{verify\_dimension\_reconciliation.py}, \texttt{verify\_edge\_gauge\_spectrum.py}, and the three earlier dimension verifiers.
\end{itemize}
...
Static carrier dimension four; $\Box$ nullity reading & REJECTED; bulk/time direction OPEN & \texttt{verify\_dimension\_reconciliation.py}; \texttt{verify\_edge\_gauge\_spectrum.py}\\
...
Mass corrections: conditional $p=4$ Morrey pattern (Eq.~\ref{eq:morrey_alpha}) & Numerically exact formula & \textbf{Pattern/Open}$^\dagger$ \\
\midrule
...
Finite spectral moments $(c_0,c_1,c_2) = (2640,14880,55920)$, $(A_0,A_1,A_2) = (11,62,233)$ & Exact & \textbf{Derived} \\
...
\bottomrule
\end{tabular}
\caption[Summary of predictions.]{Legacy summary, subject to the binding audited ledger in Table~\ref{tab:audited_status}.  ``Derived'' = deduced at the explicitly stated finite scope; ``Structural'' = requires a named identification axiom; ``Pattern'' = reproducible formula without a selecting derivation.  $\dagger$Finite summability and the fourth moment do not select $p=4$ or $d_{\mathrm{ST}}=4$; the mass-correction exponent is a pattern.  $\ddagger$The scalar-response theorem is exact on the discrete 600-cell; the continuum PPN interpretation is not claimed.  $\S$The vertex census does not fix a common normalized matter trace: a real SM generation yields $5:3:3$, while $8:5:2$ remains a pattern.}
\label{tab:summary}
\end{table}
...
\item \textbf{Gravity and dimension.} The exact discrete scalar-response identity $\gamma_{\mathrm{disc}} = 1$ and the stated polarization count are finite results.  The verified carrier is the three-dimensional boundary of the 600-cell.  Its K\"ahler--Dirac heat flow never reaches four, and neither tested unwarped nor warped finite product has a stable 4D plateau.  The former vertex-index interpretation is \textbf{REJECTED}; a separate bulk/time direction remains open.
...
\item \textbf{Mass correction unification.} The three cascade formulae can be parametrized by labels $d_{\text{eff}}=0,1,4$ and a conditional $p=4$ Morrey expression.  A finite matrix is summable in every Schatten class, and $\operatorname{Tr}D^4$ does not select a four-dimensional Sobolev energy.  Thus the unification and the $3/4$ exponent are \textbf{PATTERN/OPEN}; their numerical success is not a dimension derivation.
\end{enumerate}
