...
From $\Box$ on the 120 vertices: the speed of light $c^2 = a_1 = 5$ (the ratio of cross to fiber spectral gaps), a discrete mass spectrum in $\mathbb{Z}[\phi]$, and the McKay mass-distance correspondence on the $E_8$ Dynkin diagram ($r = 0.93$).  From $\Box$ on the 720 edges: $\ker(\Box_1) = \rho_0 \oplus 2\rho_5$ (dimension~$13$), whose 12 gauge modes decompose as $1 + 3 + 8 = \mathfrak{u}(1) \oplus \mathfrak{su}(2) \oplus \mathfrak{su}(3)$ via the $A_5$ fiber permutation---the Standard Model gauge group, derived from spectral data.  Extending $\Box$ to the full simplicial complex, the alternating sum of kernel dimensions $\sum(-1)^p\dim\ker(\Box_p) = 9 - 13 + 1 - 1 = -4 = -(a_1-1)$ is a fibration-invariant spectral index: the spacetime dimension $d_{\mathrm{ST}} = 4$ is not postulated but derived from the wave operator.  All three couplings follow algebraically: $\alpha^{-1} = 137.036$ (0.0001\%), $\alpha_s = 1/(2\phi^3)$ (0.11\%), $\sin^2\theta_W = 6/26$ (0.19\%).

Galois invariance on $\mathbb{Z}[\phi]$ forces exactly three generations.  Nine fermion masses $m_f = m_e\,\phi^{5a+6b+\delta}$, with quantum numbers $(a,b)$ uniquely constructed from the McKay graph, achieve $\chi^2 = 2.2$ over 8~degrees of freedom ($p = 0.97$).  CKM and PMNS mixing angles, CP phases ($\delta_{\text{CKM}} = \arctan\sqrt{5}$, $\theta_{\text{QCD}} = 0$), the Higgs mass ratio $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$ (0.008\% using experimental $m_W$; 0.60\% using the framework's own $m_W$), and the hierarchy $m_e/m_P = \alpha^{4\phi^2}$ (0.24\%) are all derived within the stated assumptions.  The spectral action on the 600-cell simplicial complex reproduces the discrete coefficient structure of the bosonic sector (cosmological, Einstein--Hilbert, and Yang--Mills terms), with the discrete Ricci curvature vanishing (Ollivier-flat vacuum) and gravitational stiffness $a_1^2 = c^4$; the controlled continuum identification remains open.
...
\end{equation}
Its positive root is the golden ratio $\phi = (1+\sqrt{5})/2$: the simplest number that equals one plus its own inverse.  We show that this single equation---interpreted as a fusion rule for topological excitations---determines a unique integer $a_1 = 5$, from which the spacetime dimension $d_{\mathrm{ST}} = a_1 - 1 = 4$, the gauge group, coupling constants, three fermion generations, nine mass ratios, all mixing angles, CP phases, and the Higgs-to-$W$ mass ratio of the Standard Model follow as algebraic consequences.  The dimensionless sector is fixed by $a_1$ alone; dimensional comparisons use the electron mass $m_e$ as anchor, and electroweak-scale matching inputs are stated explicitly where they enter.
...
\begin{enumerate}
\item[\textup{(F1)}] \textbf{Foundational assumptions.}  The Fibonacci fusion rule $\tau \otimes \tau = \mathbf{1} \oplus \tau$ is adopted as the \emph{minimal non-abelian self-referential seed}: abelian one-object fusion rules do not generate a non-trivial hierarchy, while richer fusion categories require additional simple objects as extra input.  The bootstrap condition $d_1(a_1)=\phi(a_1)$ is the corresponding vacuum-selection postulate.  These are the irreducible starting assumptions of the framework.
\item[\textup{(F2)}] \textbf{Exact discrete results.}  Once (F1) is accepted, the uniqueness of $a_1=5$, the 600-cell spectral data, the variational selection of $\Box$, the representation-theoretic gauge decomposition, the constructive $(a,b)$ assignment, and the exact discrete scalar-response theorem are theorems or exact finite computations on a finite simplicial complex.
\item[\textup{(F3)}] \textbf{Structural physical identifications.}  Some passages from the discrete mathematics to physical language are not standalone theorems but consistency-guided identifications, often supported by standard continuum precedent.  These include interpreting the fiber/cross anisotropy as temporal/spatial, reading the tree-level coefficient in Eq.~\eqref{eq:alpha} as the electromagnetic coupling via Hopf/Kaluza--Klein geometry, identifying the locally selected $A_5$ Galois pair $(\mathbf{3},\mathbf{3'})$ with the $W$--Higgs mass ratio, and reading the electroweak exponent from the neutral IR sector selected by adjacency blindness and refined by $\Box$.  The fermion sector should likewise be read as an \emph{emergent flavor hierarchy}: the discrete exponents define charge-like suppression data, while the final Yukawa/Lagrangian interpretation is a continuum completion problem rather than an exact theorem.
\item[\textup{(F4)}] \textbf{Open problems.}  A controlled continuum completion of the bosonic spectral action, a derivation of the RG matching from the same action, a local nonabelian gauge connection with continuum bracket, and a rigorous passage from the discrete graviton observables to 4D PPN/general-relativistic physics remain open.
\end{enumerate}
The main claim of the paper is therefore not that the full continuum Standard Model has already been derived in complete rigor, but that a tightly constrained \emph{discrete precursor} is selected from a minimal self-referential seed and reproduces a large set of Standard-Model data with sharply delimited additional assumptions.  In particular, throughout the paper we use ``derived'' only for exact discrete statements, and we flag separately those points where the physical interpretation still requires a continuum dictionary.
...
\[
\boxed{\sum_{p=0}^{3} (-1)^p \dim\ker(\Box_p) \;=\; 9 - 13 + 1 - 1 \;=\; -4 \;=\; -(a_1 - 1) \;=\; -d_{\mathrm{ST}}}
\]
provides a spectral index of the $\Box$ hierarchy, invariant across all Hopf fibrations (verified on all~6 distinct fibrations).  This index is distinct from the Euler characteristic ($\chi(S^3) = 0$): the spacetime dimension $d_{\mathrm{ST}} = a_1 - 1 = 4$ is not an input but a derived consequence of the wave operator spectrum.  The gauge content $1 + 3 + 8 = 12$ emerges exclusively at $p = 1$, while $c^2 = a_1$ is a vertex-level identity.  The trace ratios $\operatorname{Tr}(\Box_p^2)/\operatorname{Tr}(\Box_0^2)$ equal $45 = 9\,a_1$ ($p=1$) and $34$ ($p=2$), both exact integers.
...
The number of fermion generations is one of the basic unexplained integers of the Standard Model.  In the present framework the cleanest exact statement is slightly more modest than ``the Standard Model has been derived'': the discrete arithmetic and chirality structure select exactly three \emph{stable chiral flavor slots}.  Interpreting these slots as the three observed fermion generations is then the physical dictionary used in the rest of the paper.

\begin{theorem}[Generation-count theorem on the chiral unit line]
On the $a = 1$ mass line of $\mathbb{Z}[\phi]$, exactly three values of $b$ yield stable unit sectors: $b \in \{0, 1, 2\}$.  Equivalently, the unique odd/chiral line supporting multiple stable unit states contains exactly three such states.  The number three is then a rigid arithmetic property of the Fibonacci unit structure and cannot be adjusted by any continuous parameter.
\end{theorem}
...
\textbf{Step 5 (Conclusion).} Exactly three stable chiral unit sectors on the $a = 1$ line:
\begin{center}
\begin{tabular}{ccccl}
\toprule
$b$ & $k$ & $z = \phi^k$ & $n = 5 + 6b$ & Flavor slot \\
\midrule
...
\textbf{Physical reading.}  The theorem above is an exact arithmetic statement about the discrete precursor.  Its physical interpretation is that the three stable chiral unit sectors provide the three \emph{candidate family slots} of the low-energy theory.  In flavor-model language, the integers $n = 5a+6b$ are best viewed as emergent suppression charges, with $\phi^{-1}$ playing the role of the hierarchy parameter.  The claim that these three slots are realized as the observed three generations is therefore a structural identification supported by the later mass and mixing concordances, rather than a separate standalone theorem about continuum fermion fields.
...
Five independent confirmations (soliton spectral splitting, representation branching, vertex decomposition, Nyquist sampling on the Hopf base, chirality/units) all yield the same discrete family count $N_{\text{gen}} = 3$; see Supplementary Material~S4 for details.  Standard anomaly cancellation is satisfied per generation and therefore does not explain the value three by itself; here the value is instead tied to the arithmetic of the chiral unit line.
...
\end{equation}
where $(a,b) \in \mathbb{Z}^2$ are discrete quantum numbers and $m_e$ is the electron mass (the sole mass-scale input).  The intersection numbers $a_1 = 5$ and $b_1 = 6$ set the mass lattice.  In physical terms, the exponent
\[
q_f \equiv n_f + \delta_1 + \delta_2
\]
should be read as an \emph{emergent flavor charge}: the hierarchy parameter is $\epsilon = \phi^{-1}$ and $m_f/m_e = \epsilon^{-q_f}$.  What is exact at the discrete level is the charge lattice $n_f = 5a+6b$ and the arithmetic structure of the corrections.  What remains structural is the final continuum Yukawa interpretation of these exponents.  The tree-level correction $\delta_1$ is derived in Section~\ref{sec:normlog}; the one-loop QED correction $\delta_2 = \sqrt{2/a_1}\,\alpha\,\delta_1$ is derived in Section~\ref{sec:oneloop}.
...
The generation-count theorem selects $k \in \{0, 2, 3\}$ on the $a = 1$ line, giving $n = \{5, 11, 17\}$. The electron ($k = 0$) serves as the mass-scale anchor: its exponent $n_e = 0$ corresponds to $(a,b) = (0,0)$, with the $a = 1$ line value $n = 5$ absorbed into the definition of $m_e$. The effective lepton exponents for generation offsets are thus $n_{\text{lep}} = \{0, 11, 17\}$.  In the flavor-charge reading, these are the three baseline charges of the three chiral family slots.
...
\end{equation}
These offsets should be read as \emph{relative flavor-charge splittings}: once the three baseline lepton charges are fixed, the quark sectors are obtained by adding a universal up-type shift and a constrained down-type shift.
...
\textbf{Complete derivation chain.} The discrete precursor fixes the nine mass exponents in five steps: (i)~baseline lepton charges from the Fibonacci mass formula (Eq.~\eqref{eq:fibonacci_mass}), (ii)~the constant offset $a_1 - F(3) = 3$ from Galois conjugation $\tau \leftrightarrow u$ (Eq.~\eqref{eq:delta_up}), (iii)~the Casimir eigenvalue $g(g+1)$ from the $S^2$ Laplacian on the Hopf base with $l_{\max} = 2$ (Section~\ref{sec:generations}), (iv)~the sign $\pm g(g+1)$ from minimal coupling to the Hopf curvature with coefficient $2T_3$, and (v)~the sum rule $S(g) = a_1 + N_{\text{gen}} \cdot \sigma(g)$ from the constraint $\sum S = |2T| = 24$ (Eq.~\eqref{eq:sum_rule}), where the permutation $\sigma = (0\;1)(2)$ is structural---selected from six McKay-compatible candidates by the requirement that all $S$-values be framework invariants (Remark below Eq.~\eqref{eq:sum_rule}). The down-type offsets follow algebraically: $\delta_{\text{down}} = S - \delta_{\text{up}}$.  Thus, beyond the overall scale $m_e$, the framework fixes a highly constrained pattern of emergent flavor charges; the remaining continuum interpretation is the embedding of these charges into the Yukawa sector.
...
This section should be read in the same two-layer sense as the fermion sector.  What is exact on the discrete side is the existence of a neutral spectral selector, the arithmetic values of the couplings, and a rigid set of mass-ratio candidates.  Reading the corresponding exponents as the physical electroweak rung and boson masses is a \emph{structural} step, constrained both by the internal spectrum and by the external Standard Model running data.
...
The neutrino sector is one of the places where the framework remains predictive while still requiring a continuum dictionary.  The exact discrete input is a distinguished spectral asymmetry and a rank structure in the Galois kernel; the physical reading of these data as Majorana neutrino masses and a low-scale seesaw is structural, though tightly constrained.  The \emph{adjacency spectral asymmetry}, defined as the difference of multiplicities at the extremal rational eigenvalues, is:
\begin{equation}
...
This section establishes a discrete Regge/DEC-like gravitational precursor, not a complete derivation of continuum general relativity.  The exact statements concern Hodge decomposition, gauge redundancy, scalar response, and spectral stiffness on the 600-cell.  Their interpretation as continuum gravitational physics is intentionally more cautious and is treated as a controlled analogy or open completion problem when appropriate.
...
In the transverse (coexact) gauge, the natural discrete Einstein-like response equation on the 600-cell is:
\begin{equation}
\boxed{C \cdot h_T = 8\pi G \cdot T_T}
\label{eq:gravity}
\end{equation}
where $C = d_1^T d_1$ is the coexact Laplacian restricted to the physical sector, $h_T$ is the transverse (coexact) edge perturbation, and $T_T$ is the transverse stress-energy on edges. This is the discrete analog of the linearized Einstein equation in transverse-traceless gauge: $\Box\, h_{TT} = -16\pi G\, T_{TT}$.  The equation is exact as a discrete response law; what remains open is the rigorous identification of its long-wavelength sector with 4D linearized GR.
...
The algebraic framework also suggests several extensions beyond the core discrete Standard-Model precursor.  These are intentionally separated from the main claims because their physical interpretation is less mature than the gauge/flavor core.  Full details are in Supplementary Material Sections~S11--S14; we summarize the key results here, distinguishing exact internal no-go statements from more speculative phenomenological readings.
...
Using the taxonomy introduced in Section~\ref{sec:foundations}, we now summarize the evidential status of the main quantitative claims.  To guard against post-hoc numerological fitting, we distinguish three levels in the table below: (i)~\textbf{Rigorous derivation} ($a_1 = 5 \to$ result via established mathematics on the discrete complex, no residual choices); (ii)~\textbf{Structural} (framework constants only, but with a physically motivated identification step or continuum dictionary); (iii)~\textbf{Pattern/Open} where a clean algebraic formula exists but a unique dynamical derivation is still missing.  In particular, the family count and the fermion mass sector should be read as a \emph{derived discrete flavor hierarchy} together with a \emph{structural continuum flavor interpretation}.  Table~\ref{tab:summary} categorizes every result:
...
$m_Z$ ($n\!=\!a_1^2$ from neutral IR sector $+$ RG bootstrap, Eq.~\ref{eq:rg_cost}), $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$, $m_e/m_P = \alpha^{4\phi^2}$ & 0.008--0.24\% & \textbf{Structural/Derived mix} \\
\midrule
\multicolumn{3}{l}{\textit{Structural}} \\
Gauge group $SU(3) \times SU(2) \times U(1)$; discrete family count $N_{\text{gen}} = 3$ (vertex + edge, Sec.~\ref{sec:edge_gauge}) & Exact / structural reading & \textbf{Derived/Structural mix} \\
...
All 9 fermions (with vertex correction) & $\chi^2/\text{dof} = 0.28$ & \textbf{Derived/Structural mix} \\
...
What remains open is the absolute scale conversion---why the lattice spacing is $1/(m_e\,\phi^{25})$ rather than, say, $1/m_P$---which is the hierarchy problem in discrete form.  The 600-cell spectral data are pure numbers in lattice units; no functional of these data can produce a dimensionful quantity without at least one dimensionful input.  Given one such input ($m_e$ or equivalently $m_P$), the spectrum determines all other scales through derived ratios ($m_e/m_P = \alpha^{4\phi^2}$ to $0.24\%$, $m_Z/m_e = \phi^{a_1^2} \cdot R$ to $0.09\%$).  What is new is that the exponent $25$ itself is no longer supported only by the RG window: it is the unique adjacency-blind IR rank and the unique $\Box$-refined neutral count.  The remaining structural step is only the reading of this spectral selector as the mass-ladder exponent.  The hierarchy $m_e/m_P \sim 10^{-22}$ is $\alpha^{10.5}$, where both $1/137$ and $10.5$ are moderate numbers derived from~$a_1$; the large ratio arises from exponentiation, not fine-tuning.

\subsection{Conclusions}

We have shown that a substantial part of Standard Model structure---its gauge group, coupling constants, fermion content, flavor hierarchies, mixing angles, CP structure, and discrete gravitational analogues---can be systematically organized from a single integer $a_1 = 5$.
...
\item \textbf{Fermion masses and uniqueness.}  The Fibonacci mass formula $m_f = m_e \phi^{5a+6b+\delta_1+\delta_2}$ with zero-parameter corrections (norm-log for quarks, Galois conjugate for leptons, Verlinde radiative corrections with $c_{\mathrm{eff}} = \sqrt{2/a_1}\,(1-b_1\alpha^2)$ from the $S$-matrix prefactor and fusion matrix Frobenius norm) achieves $\chi^2 = 2.2/8$ dof ($p = 0.97$, RMS pull $0.53\sigma$) across all 9 fermions.  Conceptually, this sector is best read as an emergent flavor theory: the discrete data fix a lattice of suppression charges, while the final Yukawa interpretation is structural rather than a separate exact theorem.  The $(a,b)$ assignment is no longer only a bounded-search observation: Theorem~\ref{thm:uniqueness} gives a constructive derivation from McKay main-chain weights, minimal local edge lifts in $\mathbb{Z}[\phi]$, branch identification from affine-$E_8$ geometry and chirality, and the prime-sector Galois pairing $z_b = \phi\,\sigma(z_t)$.  The older conditions (C1)--(C2) survive as arithmetic corollaries and as a computational cross-check inside the bounded domain.  The McKay mass correspondence forces the main-chain weights to $[N_{\text{gen}}, F(3), b_1, 0, a_1]$ uniquely (Theorem~\ref{thm:mckay_mass}).  The three correction regimes (logarithmic, power-law, constant) are unified by the Morrey--Sobolev embedding at effective dimension $d_{\text{eff}} = 0, 1, d_{\text{ST}}$, with H\"{o}lder exponent $\alpha = 1 - d_{\text{eff}}/d_{\text{ST}}$ (Eq.~\ref{eq:morrey_alpha}).  The Higgs-to-$W$ mass ratio $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$ is now more constrained than a bare spectral coincidence: $\phi^2$ is the ratio of the unique positive local $A_5$-equivariant Laplacian eigenvalues on the Galois pair $(\mathbf{3},\mathbf{3'})$, while the remaining correction is the already-delimited finite-triple term $-16\alpha\phi$.
