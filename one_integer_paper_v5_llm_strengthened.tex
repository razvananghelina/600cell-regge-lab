...
This section records finite Regge/DEC-like algebraic structures, not a derivation of continuum gravity.  The exact statements concern the Hodge decomposition, scalar graph response, and spectral edge-weight stiffness on the 600-cell.  Gauge redundancy, metric variables, stress-energy coupling, and their continuum gravitational interpretation require additional dictionaries and remain open.
...
\item \textbf{Gravity.} The finite Hodge decomposition, exact discrete scalar response, coexact/scalar multiplicity ratio, and positive-definite edge-weight Hessian remain derived algebraic facts.  In addition, the round quaternionic Hopf geometry now supplies a genuine $\mathrm{Sym}^2_0(T^*S^3)$ carrier with two distinct five-dimensional homogeneous TT eigenspaces.  This closes the tensor-type gap but not the dynamics: no selected coupling/action, diffeomorphism quotient, Lorentzian propagator, universal stress-tensor coupling, fixed-Regge transfer, or 4D continuum limit has been derived.  Emergent gravity, PPN, and nonlinear GR remain open.
