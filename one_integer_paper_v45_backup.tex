...
From $a_1$ we obtain $\phi = (1+\sqrt{5})/2$, the 600-cell ($120$ vertices), and $E_8$ via the icosian construction.  We prove $N_{\text{gen}} = 3$ from Galois invariance on $\mathbb{Z}[\phi]$, derive the gauge group $SU(3)\!\times\!SU(2)\!\times\!U(1)$ from two independent routes---the $A_5$ decomposition of the vertex figure, and the kernel of the wave operator on the edge space ($\ker(\Box_1) = \rho_0 \oplus 2\rho_5$, the $E_8$ branching node)---and obtain all three couplings algebraically: $\alpha^{-1} = 137.036$ (0.0001\%), $\alpha_s = 1/(2\phi^3) = 0.1180$ (0.11\%), $\sin^2\theta_W = 6/26$ (0.19\%).  Nine fermion masses follow from $m_f = m_e\,\phi^{5a+6b+\delta_1+\delta_2}$ with zero-parameter norm-log, Galois-conjugate, and Verlinde radiative corrections ($c_{\mathrm{eff}} = \sqrt{2/a_1}\,(1-b_1\alpha^2)$, one-loop from the $S$-matrix prefactor, two-loop vertex from the fusion matrix Frobenius norm $\|N_{1/2}\|_F^2 = b_1$), achieving $\chi^2 = 2.2/8$ dof ($p = 0.97$).  CKM and PMNS mixing angles, CP phases ($\delta_{\text{CKM}} = \arctan\sqrt{5}$, $\delta_{\text{PMNS}} = 3\arctan\sqrt{5}$), and $\theta_{\text{QCD}} = 0$ are derived within the stated assumptions.  The bare CKM exponents $\{3,7,12\}$ satisfy three algebraic identities whose intersection singles out $a_1 = 5$ (thirteenth characterisation); the Cayley--CKM vacuum offset $\delta = 1$ provides a fourteenth.  The Higgs mass ratio $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$ (0.008\%) and the hierarchy $m_e/m_P = \alpha^{4\phi^2}$ (0.24\%) connect electroweak and gravitational scales through a single identity $z_{\text{Planck}} = 1/\alpha_s + 2$.  The spectral action on the 600-cell simplicial complex reproduces the coefficient pattern of the bosonic sector, with all discrete coefficients determined by~$a_1$.
...
The framework rests on a single principle: \emph{self-referential consistency}.  The equation $x^2 = x + 1$---the requirement that the whole equal its part plus unity---generates the golden ratio~$\phi$, whose Fibonacci fusion rule $\tau \otimes \tau = \mathbf{1} \oplus \tau$ defines the simplest non-trivial topological quantum field theory.  Demanding that the algebraic parameter of this theory equal its own quantum dimension selects $a_1 = 5$ uniquely (Theorem~\ref{thm:bootstrap}).  From there, the 600-cell, $E_8$, and the Standard Model follow without further input.  Fermion masses are powers of~$\phi$ because each mass scale is a level of the same self-similar structure; the mass formula $m_f = m_e\,\phi^n$ is the fractal decomposition $\phi^n = F_n\,\phi + F_{n-1}$ made physical.  Dark matter is not an addition: it is the second root of the same equation, $\phi' = -1/\phi$, the Galois conjugate that shares the algebraic structure but lacks a real electromagnetic coupling.  The sole dimensional input~$m_e$ anchors the fractal to a physical scale.
...
\end{equation}
This coefficient links the Weinberg angle to the number of generations: it equals $\tfrac{2}{3}\sin^2\theta_W$, or equivalently $d_{\text{ST}}/(a_1^2+1)$, the ratio of the spacetime dimension $d_{\text{ST}} = a_1-1 = 4$ to the modular invariant $a_1^2+1 = 26$.  Both $\sin^2\theta_W = b_1/(a_1^2+1)$ (Eq.~\eqref{eq:weinberg}) and $N_{\text{gen}} = 3$ (Section~\ref{sec:generations}) are derived from $a_1 = 5$, so $C$ is \emph{algebraically determined} with zero free parameters.  Its appearance as the mass correction coefficient reflects the coupling between the spacetime spectral triple ($d_{\text{ST}} = 4$) and the internal space ($a_1^2+1 = 26$ independent correction channels); equivalently, $C = \mathrm{rank}(E_8)/(2(a_1^2+1))$, since $\mathrm{rank}(E_8) = 2\,d_{\text{ST}} = 8$.
...
\textbf{Assessment.} The full correction formula has \emph{zero free parameters}: the quark coefficient $C = 4/(a_1^2+1)$ is determined by $a_1$, the $1/\phi$ suppression is the Galois conjugate ratio, the lepton exponent $k = 3/4$ and coefficient $c_\ell = C\phi^3/4$ are both derived from $\mathrm{Tr}(\phi^3) = \dim(\mathrm{ST}) = 4$, the one-loop coefficient $c_{\mathrm{bare}} = \sqrt{2/a_1}$ is derived from the Verlinde $S$-matrix, and the vertex correction coefficient $c_V = b_1 = \|N_{1/2}\|_F^2$ from the fusion matrix Frobenius norm (Section~\ref{sec:oneloop}).  Each correction mechanism has a structural origin: $\ln|N(z)|$ is the Arakelov height over $\mathbb{Q}(\sqrt{5})$~\eqref{eq:arakelov}, the $d$-quark correction $-2/a_1$ is the Galois representation cross-term on $A_5$ five-cycles~\eqref{eq:delta_d_galois}, the unit-sector correction $-N_{\text{gen}} C/\phi^2$ is derived from the Galois anti-symmetric Cayley eigenvalue $|G(\rho_4)| = 3 = N_{\text{gen}}$ (Proposition~\ref{prop:galois_antisym}), $\delta_u = 0$ follows from $G(\rho_2) < 0$ (sign selection), the lepton exponent $3/4 = 1 - 1/d_{\text{ST}}$ is the Morrey--Sobolev H\"{o}lder regularity for the one-dimensional McKay graph in $d_{\text{ST}} = 4$ ambient dimensions, with the bound saturated because $G_k = 0$ for WHITE nodes removes all spectral constraints beyond regularity (Step~3 above), and the lepton coefficient $c_\ell = C\phi^3/4$ is uniquely determined by $\mathrm{Tr}(\phi^3) = d_{\text{ST}}$.  All 8 independent tree-level corrections plus the one-loop coefficient are derived from first principles.  The saturation of the Morrey bound for leptons follows from a two-layer argument: (i)~$G_k = 0$ at \emph{all orders} for WHITE nodes (since $\chi_k(10a) \in \mathbb{Q}$ implies $\sigma(\chi_k) = \chi_k$, eliminating all spectral mechanisms), and (ii)~$|N(z)| \leq 1$ for leptons (eliminating the Arakelov height and Galois cross-term mechanisms available to the $d$ and $b$ quarks, which also sit on WHITE nodes).  With both layers of constraints absent, the only remaining information is $d_{\text{ST}}$-summability of the spectral triple, and the Morrey--Sobolev bound $\alpha \leq 3/4$ becomes tight.  Including the full radiative correction $\delta_2 = c_{\mathrm{eff}}\,\alpha\,\delta_1$ with $c_{\mathrm{eff}} = \sqrt{2/a_1}\,(1 - b_1\alpha^2)$, the global fit gives $\chi^2 = 2.24$ for $8$ degrees of freedom ($p = 0.97$), with RMS pull $0.53\,\sigma$ (Table~\ref{tab:corrected_masses_normlog}).  The muon pull drops from $+2.55\,\sigma$ (one-loop only) to $-0.03\,\sigma$ once the two-loop vertex correction is included; the key is that the self-energy (which worsens the fit) and the vertex correction (which screens the effective coupling) are physically distinct effects with opposite signs.  The vertex coefficient $b_1 = \|N_{1/2}\|_F^2 = 6$ is derived from the fusion matrix Frobenius norm (Section~\ref{sec:oneloop}), not fitted.  The Koide mass relation $Q = 2/3$ provides an independent consistency check: $Q(3/4) = 0.66668$ ($0.002\%$ from $2/3$), and the mass-vector angle with $(1,1,1)$ is $45.001^\circ$.
...
\begin{remark}
The mass-splitting ratio~\eqref{eq:dm_ratio} is classified as \textbf{derived}: (i)~the exponent $n = 3$ follows from the kernel spectral zeta at conformal dimension $s = (d{-}1)/d = 3/4$; (ii)~$\alpha$ enters to the first power because the perturbation is an inner fluctuation on the \emph{finite} spectral triple (no loop integral, no $(4\pi)^2$ suppression); (iii)~the coupling is $\alpha_{\text{EM}}$ because $U(1)_Y$ is the unique gauge factor that mixes the Galois kernel partners $\rho_1$, $\rho_8$; (iv)~the coefficient $c = 1$ follows from equal hypercharge of these partners (derived from anomaly cancellation, Section~\ref{sec:anomaly}).  The rank-2 property $m_1 = 0$ is also \textbf{derived}, from three independent arguments: (a)~the trivial irrep $\rho_0$ has Cayley Laplacian eigenvalue $L_0 = 0$, so $M_R = 0$ and it does not participate in the seesaw; (b)~$\rho_0$ carries no gauge quantum numbers and cannot form a Yukawa coupling to the Higgs; (c)~$\rho_0$ is Galois-fixed ($\sigma(\rho_0) = \rho_0$), so it has no partner for Majorana pairing.  The predictions $\sum m_\nu = 58.2$~meV, $m_\beta = 8.8$~meV, and normal ordering are \emph{falsifiable} by JUNO ($\sim$2028), DESI/Euclid, KATRIN/Project~8, and next-generation $0\nu\beta\beta$ experiments within 5~years.
\end{remark}
...
\item \textbf{Spectral action normalization.} The exact discrete coefficients $(c_0,c_1,c_2) = (2640,14880,55920)$, the reduced triple $(11,62,233)$, and the Diophantine identity~\eqref{eq:seeley_diophantine} are established.  The gauge prefactors $(8/15,1/3,2/15) = (|Z(2I)| : a_1 : \mathrm{rank}(E_8))/(N_{\text{gen}} \cdot a_1)$ are explained by the vertex-type decomposition under $Q_8 \subset 2T \subset 2I$ (Eq.~\ref{eq:gauge_prefactors}), with $Q_8$'s abelian role now derived from three coordinate-free criteria (commutator centrality, Galois trace classification, character-weight vanishing on broken sectors).  What remains open is a controlled continuum identification with the full bosonic Standard Model action.
\item \textbf{Gravity.} The linearized field equation, the exact discrete scalar-response identity $\gamma_{\mathrm{disc}} = 1$, and the correct graviton polarization count ($R \to 2$) are derived on the discrete model; the 4D nonlinear completion and any continuum PPN interpretation remain open.
...
\item \textbf{Fermion masses and uniqueness.}  The Fibonacci mass formula $m_f = m_e \phi^{5a+6b+\delta_1+\delta_2}$ with zero-parameter corrections (norm-log for quarks, Galois conjugate for leptons, Verlinde radiative corrections with $c_{\mathrm{eff}} = \sqrt{2/a_1}\,(1-b_1\alpha^2)$ from the $S$-matrix prefactor and fusion matrix Frobenius norm) achieves $\chi^2 = 2.2/8$ dof ($p = 0.97$, RMS pull $0.53\sigma$) across all 9 fermions.  The $(a,b)$ assignment is no longer only a bounded-search observation: Theorem~\ref{thm:uniqueness} gives a constructive derivation from McKay main-chain weights, minimal local edge lifts in $\mathbb{Z}[\phi]$, branch identification from affine-$E_8$ geometry and chirality, and the prime-sector Galois pairing $z_b = \phi\,\sigma(z_t)$.  The older conditions (C1)--(C2) survive as arithmetic corollaries and as a computational cross-check inside the bounded domain.  The McKay mass correspondence forces the main-chain weights to $[N_{\text{gen}}, F(3), b_1, 0, a_1]$ uniquely (Theorem~\ref{thm:mckay_mass}).  The three correction regimes (logarithmic, power-law, constant) are unified by the Morrey--Sobolev embedding at effective dimension $d_{\text{eff}} = 0, 1, d_{\text{ST}}$, with H\"{o}lder exponent $\alpha = 1 - d_{\text{eff}}/d_{\text{ST}}$ (Eq.~\ref{eq:morrey_alpha}).  The Higgs-to-$W$ mass ratio $m_H^2/m_W^2 = \phi^2 - 16\alpha\phi$ follows from $L(\mathbf{3'})/L(\mathbf{3}) = \phi^2$ with a derived unit coefficient (Proposition~\ref{prop:higgs_coeff}).
