...
endpoint now has a derived all-form conical
coefficient~\cite{cheeger1983,fursaevmiele1997}, and at equal volume
the same $A_2$ narrowly prefers round over fixed Regge,
$-78.9568352087\ldots<-78.8719985927\ldots$.  Applying only the linear
smooth-Regge deficit term gives the opposite ordering and is refuted.  The
de Rham transmittal face term on the preregistered affine interpolation has an
exact $u(1-u)$ factor and cannot be omitted.  With that term included, all
$200$ interior points of the frozen five-order quadrature grid lie strictly
above the round endpoint and decrease toward it.  This is a numerical pattern,
not a continuum interval theorem.  The finite ratio
$c_1/(2c_0)=31/11$ scales as inverse length squared and leaves a free
...
\item \textbf{Gravity.} The finite Hodge decomposition, exact discrete scalar response, coexact/scalar multiplicity ratio, and positive-definite edge-weight Hessian remain derived algebraic facts.  The round quaternionic Hopf geometry supplies a genuine $\mathrm{Sym}^2_0(T^*S^3)$ carrier with two distinct five-dimensional homogeneous TT eigenspaces.  Moreover, the ordinary continuum de Rham heat coefficient $A_2=-(2/3)\int R\,dV$ has the nondegenerate Hopf Hessian $(8/3)I_5$ and uniquely minimizes at the round metric over all fixed-volume left-invariant $SU(2)$ metrics.  The exact all-form cone coefficient also prefers the round endpoint over the fixed Regge endpoint at equal volume, and the preregistered interior grid is a converged monotone pattern in the same direction.  This is a derived conditional shape and endpoint selector plus an interior numerical pattern, not a selected finite action: a continuum certificate for the smooth--Regge path, absolute cutoff/normalization, diffeomorphism quotient, Lorentzian propagator, universal stress-tensor coupling, and 4D completion remain open.  Emergent gravity, PPN, and nonlinear GR remain open.
